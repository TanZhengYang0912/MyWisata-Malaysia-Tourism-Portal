...
function fallbackImage(categoryLabel: string) {
  return (
    <div className="flex h-full min-h-44 flex-col items-center justify-center bg-secondary text-primary">
      ...
      <span className="mt-0.5 text-[10px] text-muted-foreground/60">Photo coming soon</span>
    </div>
  );
}
    ...
    <article className="group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition duration-300 hover:-translate-y-1 hover:border-primary/25 hover:shadow-md">
      <Link href={detailHref} className="relative block aspect-[4/3] overflow-hidden bg-secondary" aria-label={`View ${product.name}`}>
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-2 bg-gradient-to-t from-black/80 via-black/20 to-transparent px-4 pb-3 pt-12">
          ...
          {product.featured && <span className="rounded-full bg-amber-400 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-black">Featured</span>}
        </div>
      </Link>
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="shrink-0 text-right">
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground/60">From</p>
    ...
    <section id="full-menu" aria-labelledby="full-menu-title" className="rounded-3xl border border-primary/10 bg-card p-5 shadow-sm sm:p-7">
      <div className="flex flex-col gap-3 border-b border-border pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 id="full-menu-title" className="mt-2 text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Available at this outlet</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Explore the full menu and experiences available here, with the exact price and booking options for this location.</p>
        </div>
      ...
      {products.length === 0 ? (
        <div className="mt-6 rounded-2xl border border-dashed border-primary/20 bg-secondary/40 px-5 py-10 text-center">
          <p className="font-semibold text-foreground">The menu is being prepared</p>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">This outlet has not published a sellable item yet. Check back soon for local favourites.</p>
        </div>
      ) : (
