...
export function OutletBlockRenderer({ block, outlet, products = [], gallery = [], featuredIds = [], w = 4, h = 2, mode = 'public', selected = false, onSelect, onEditBlock, onEditEnd }: Props) {
  ...
  const content = (
    <>
      ...
      {mode === 'editor' && !model.imageUrl && ['image', 'image_text'].includes(block.type) && <div className="mt-4 flex h-40 items-center justify-center rounded-xl border-2 border-dashed border-primary/15 bg-secondary/40 text-xs font-semibold text-muted-foreground/60">Drop image here</div>}
      {editable && ['image', 'image_text'].includes(block.type) && <input aria-label={`Edit ${model.label} image URL`} value={model.imageUrl || ''} onChange={(event) => onEditBlock?.({ imageUrl: event.target.value, image: event.target.value })} onBlur={onEditEnd} onClick={(event) => event.stopPropagation()} placeholder="Paste image URL" className="mt-2 block h-9 w-full rounded-lg border border-primary/10 bg-background px-2 text-xs outline-none ring-primary/50 focus:ring-2" />}
      ...
      {(block.type === 'cta' || block.type === 'voucher_banner') && <Link className="mt-4 inline-flex rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90" href={model.buttonLink || '#featured-products'}>{model.cta || 'Explore this outlet'}</Link>}
