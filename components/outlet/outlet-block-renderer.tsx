    ...
    <div className={`relative mx-auto max-w-7xl px-6 text-white ${mode === 'public' ? 'flex min-h-[280px] flex-col justify-end py-12 sm:min-h-[340px] sm:py-14 lg:min-h-[360px] lg:py-16' : 'py-24'}`} style={{ textAlign: hero.textAlign || 'left' }}>
      ...
      {mode === 'editor' && onEditHero ? (
        <textarea
          ...
          onClick={(event) => event.stopPropagation()}
          rows={2}
          className="mt-4 block w-full max-w-2xl resize-none rounded-lg border border-white/25 bg-black/10 px-2 py-1 text-sm text-white outline-none ring-amber-300 placeholder:text-white/60 focus:ring-2"
        />
      ) : (
...
export function OutletBlockRenderer({ block, outlet, products = [], gallery = [], featuredIds = [], w = 4, h = 2, mode = 'public', selected = false, onSelect, onEditBlock, onEditEnd }: Props) {
  ...
  const mediaHeightClass = mode === 'editor' ? 'h-28' : 'h-40';
  const productMediaHeightClass = mode === 'editor' ? 'h-24' : 'h-36';
  const productCardPaddingClass = mode === 'editor' ? 'p-2' : 'p-4';
  ...
  const editorProductLimit = mode === 'editor' ? tile.columns : tile.items;
    ...
    <>
      ...
      {(!isPhoto || isPhotoStory) && editable ? (
        <textarea
          ...
          className="mt-2 block w-full resize-none rounded-lg border border-primary/10 bg-background px-2 py-1 text-sm leading-6 text-muted-foreground outline-none ring-primary/50 focus:ring-2"
        />
      ) : (!isPhoto || isPhotoStory) ? (
        model.body && <p className="mt-2 text-sm leading-6 text-muted-foreground">{model.body}</p>
      ) : null}
      {/* eslint-disable-next-line @next/next/no-img-element */}
      {model.imageUrl && block.type !== 'hero' && <img src={model.imageUrl} alt={model.title} className={`mt-4 ${mediaHeightClass} w-full rounded-xl object-cover`} />}
      {mode === 'editor' && !model.imageUrl && ['image', 'image_text'].includes(block.type) && <div className={`mt-4 flex ${mediaHeightClass} items-center justify-center rounded-xl border-2 border-dashed border-primary/15 bg-secondary/40 text-xs font-semibold text-muted-foreground/60`}>{t('ui.outlet.dropImage')}</div>}
      ...
      {block.type === 'product_grid' && (visibleProducts.length > 0 ? <div id="featured-products" className="mt-4 grid gap-3" style={{ gridTemplateColumns: `repeat(${tile.columns}, minmax(0, 1fr))` }}>{visibleProducts.slice(0, editorProductLimit).map((product) => {
        ...
        ) : <div className="flex h-full items-center justify-center text-xs font-semibold text-muted-foreground/60">{t('ui.outlet.photoComingSoon')}</div>}</div><div className={productCardPaddingClass}><div className="flex items-start justify-between gap-2"><p className="min-w-0 flex-1 break-words whitespace-normal text-sm font-bold text-foreground">{product.name}</p><p className="shrink-0 text-xs font-bold text-primary">{formatMYR(Number(product.base_price))}</p></div>{product.description && <p className="mt-1 break-words whitespace-normal text-[11px] leading-4 text-muted-foreground">{product.description}</p>}<p className="mt-2 break-words whitespace-normal text-[11px] font-semibold text-primary">{product.requires_booking ? t('ui.outlet.bookingRequired') : t('ui.outlet.availableToExplore')} <span aria-hidden="true">→</span></p></div></Link>;
