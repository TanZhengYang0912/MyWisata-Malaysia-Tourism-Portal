...
export function OutletBlockRenderer({ block, outlet, products = [], gallery = [], featuredIds = [], w = 4, h = 2, mode = 'public', selected = false, onSelect, onEditBlock, onEditEnd }: Props) {
  ...
  const content = (
    <>
      ...
      {block.type === 'review_highlight' && <p className="mt-4 rounded-xl bg-secondary px-4 py-3 text-sm text-slate-700">{'overrides' in block && block.overrides?.review ? live.review : t('ui.outlet.verifiedGuestsRecommend')}</p>}
