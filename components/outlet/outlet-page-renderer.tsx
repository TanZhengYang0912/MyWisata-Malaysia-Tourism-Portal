...
function VisitSummary({ outlet }: { outlet: OutletPageRendererProps['outlet'] }) {
  ...
  return <section className="mb-6 rounded-2xl border border-primary/10 bg-card p-5 shadow-sm" aria-label="Plan your visit">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div className="grid gap-4 text-sm text-muted-foreground sm:grid-cols-2 xl:grid-cols-4">
        <div className="flex items-start gap-3"><MapPin size={18} className="mt-0.5 shrink-0 text-primary" /><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">Location</p><p className="mt-1 font-semibold text-foreground">{address}</p></div></div>
