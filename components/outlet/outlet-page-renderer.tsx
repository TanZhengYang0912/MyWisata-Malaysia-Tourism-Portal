  ...
  return <section className="mb-6 rounded-2xl border border-primary/10 bg-card p-5 shadow-sm" aria-label={t('ui.outlet.planVisit')}>
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div className="grid gap-4 text-sm text-muted-foreground sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="flex items-start gap-3"><Clock3 size={18} className="mt-0.5 shrink-0 text-primary" /><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{t('ui.outlet.hours')}</p><p className="mt-1 leading-5">{formatHours(outlet.operating_hours, { scheduleUnavailable: t('ui.outlet.scheduleBeforeBooking'), closed: t('ui.outlet.closed'), day: (day) => t(`ui.labels.days.${day.slice(0, 3).toLowerCase()}`) })}</p></div></div>
