...
export function ReportChatButton({ chatType, threadId, className }: ReportChatButtonProps) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showFeedback("error", body?.error?.message ?? tCommon("reportConversation.submitFailed"));
        return;
      }
      setReported(true);
      setOpen(false);
      showFeedback("success", tCommon("reportConversation.success"));
    } catch {
      showFeedback("error", tCommon("reportConversation.submitFailedRetry"));
    } finally {
      setSending(false);
    }
  }
  ...
  if (reported) {
    return <span className={`text-xs text-muted-foreground ${className ?? ""}`}>{tCommon("reportConversation.reported")}</span>;
  }
  ...
  return (
    <div className={`relative ${className ?? ""}`}>
      <button
        ...
        aria-label={tCommon("reportConversation.title")}
        title={tCommon("reportConversation.report")}
      >
        <Flag size={14} />
      </button>
      {open && (
        <div className="absolute right-0 top-9 z-20 w-64 rounded-2xl border border-border bg-card p-3 shadow-lg">
          <p className="mb-2 text-sm font-semibold text-foreground">{tCommon("reportConversation.title")}</p>
          <textarea
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            placeholder={tCommon("reportConversation.reasonOptional")}
            rows={2}
            maxLength={500}
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="destructive" className="flex-1" disabled={sending} onClick={() => void submit()}>
              {sending ? tCommon("reportConversation.submitting") : tCommon("reportConversation.submit")}
            </Button>
