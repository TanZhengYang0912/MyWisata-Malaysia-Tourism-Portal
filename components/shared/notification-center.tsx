...
export function NotificationCenter({ scope = "customer", vendorId = null, categories = DEFAULT_CATEGORIES, pageSize = 15 }: NotificationCenterProps) {
  ...
  return <div>
    ...
    <div className="rounded-2xl border border-border bg-card">{items.length === 0 ? <p className="p-8 text-center text-sm text-muted-foreground">{t("notifications.noNotificationsFound", { defaultValue: "No notifications found." })}</p> : items.map((item) => <div key={item.id} className={`border-b border-border px-5 py-4 last:border-0 ${item.readAt ? "" : "bg-primary/5"}`}><button type="button" onClick={() => void markRead(item.id)} className="w-full text-left"><p className="font-semibold">{item.title}</p><p className="mt-1 text-sm text-muted-foreground">{item.body}</p><p className="mt-1 text-xs text-muted-foreground">{formatDateTime(item.createdAt, locale)}</p></button>{item.link && <Link className="mt-2 inline-block text-xs font-semibold text-primary" href={item.link}>{t("notifications.open", { defaultValue: "Open" })}</Link>}</div>)}</div>
    <div className="mt-4 flex items-center justify-between text-sm"><button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)} aria-label={t("accessibility.previousPage", { defaultValue: "Previous page" })} className="rounded-lg border border-border px-3 py-2 disabled:opacity-50">{t("actions.previous", { defaultValue: "Previous" })}</button><span aria-live="polite">{t("pagination.page", { current: formatNumber(page, locale), total: formatNumber(totalPages, locale) })}</span><button type="button" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)} aria-label={t("accessibility.nextPage", { defaultValue: "Next page" })} className="rounded-lg border border-border px-3 py-2 disabled:opacity-50">{t("actions.next", { defaultValue: "Next" })}</button></div>
  </div>;
}
