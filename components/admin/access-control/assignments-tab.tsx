...
import type { ApiEnvelope, AssignmentRecord, AuditFocus, MutationReceipt, PageResult } from "@/components/admin/access-control/types";
...
export function AssignmentsTab({ focusId, onViewAudit }: { focusId: string | null; onViewAudit: (focus: AuditFocus) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/admin/access-control/assignments?${query}`, { cache: "no-store" });
      const body = await response.json() as ApiEnvelope<PageResult<AssignmentRecord>>;
      if (!response.ok || !body.data) throw new Error(errorMessage(body, t("accessControl.errors.loadAssignments")));
      setResult(body.data);
