...
export default function CustomerCalendarView({ initialScope = "upcoming" }: { initialScope?: BookingScope } = {}) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (active) { setError(reason instanceof Error ? reason.message : tCustomer("ui.states.loadingError")); setBookings([]); setLoadedUserId(currentUser.id); }
      }
    };
    void load();
  ...
  return (
    <div className="min-h-full bg-background">
      <div className="mx-auto max-w-6xl px-4 py-2 sm:px-6 sm:py-3">
        <section aria-label={tCustomer("ui.booking.calendar")} className="overflow-hidden rounded-3xl border border-border bg-white shadow-[0_12px_32px_rgba(1,0,102,0.06)]">
          <h1 className="sr-only">{tCustomer(`ui.calendar.scopeTitles.${scope}`)}</h1>
          <div className="border-b border-slate-100">
            <div className="flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
              ...
              <div className="relative text-center">
                ...
                {monthPickerOpen && <div id="calendar-month-picker" role="dialog" aria-label={tCustomer("ui.booking.chooseDateTime")} className="absolute left-1/2 top-full z-30 mt-2 w-[min(22rem,calc(100vw-2rem))] -translate-x-1/2 rounded-2xl border border-border bg-white p-4 text-left shadow-xl">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <label className="flex items-center gap-2 text-xs font-semibold text-slate-500">
                      <span>{tCustomer("ui.labels.date")}</span>
                      <select value={monthStart.getFullYear()} onChange={(event) => selectCalendarYear(Number(event.target.value))} aria-label={tCustomer("ui.calendar.chooseYear")} className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-sm font-bold text-primary outline-none focus:border-primary focus:ring-2 focus:ring-primary/20">
            ...
            <div aria-label={tCustomer("ui.calendar.actions")} className="flex items-center justify-between gap-3 border-t border-slate-100 px-4 py-2.5 sm:px-6">
              <Link href={activityHref("orders")} className="inline-flex h-9 items-center gap-1.5 rounded-full border border-primary/20 bg-white px-3 text-xs font-bold text-primary hover:bg-secondary"><ReceiptText size={14} /> {tCustomer("ui.labels.orders")}</Link>
              <div className="flex items-center justify-end gap-2">
                <Button variant="outline" onClick={goToToday} className="h-9 rounded-full border-primary/20 bg-white px-3 text-xs text-primary hover:bg-secondary">{tCustomer("ui.calendar.today")}</Button>
            ...
            {filtersOpen && <div id="calendar-filters" className="border-t border-slate-100 px-4 py-3 sm:px-6"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-6"><label className="relative min-w-0 lg:col-span-2"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => { setSelectedDayKey(null); setQuery(event.target.value); }} placeholder={tCustomer("ui.calendar.searchBookings")} aria-label={tCustomer("ui.calendar.searchActivity")} className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50/70 pl-9 pr-3 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-[rgba(1,0,102,0.12)]" /></label><select value={scope} onChange={(event) => { setSelectedDayKey(null); setScope(event.target.value as BookingScope); }} aria-label={tCustomer("ui.calendar.bookingScope")} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-primary"><option value="upcoming">{tCustomer("ui.calendar.upcoming")}</option><option value="past">{tCustomer("ui.calendar.past")}</option><option value="all">{tCustomer("ui.calendar.allBookings")}</option></select><select value={outletId} onChange={(event) => { setSelectedDayKey(null); setOutletId(event.target.value); }} aria-label={tCustomer("ui.calendar.bookingOutlet")} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-primary"><option value="all">{tCustomer("ui.calendar.allOutlets")}</option>{outlets.map((outlet) => <option key={outlet.id} value={outlet.id}>{outlet.name}</option>)}</select><select value={activityId} onChange={(event) => { setSelectedDayKey(null); setActivityId(event.target.value); }} aria-label={tCustomer("ui.calendar.bookingActivity")} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-primary"><option value="all">{tCustomer("ui.calendar.allActivities")}</option>{activityOptions.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select><input type="date" value={from} onChange={(event) => { setSelectedDayKey(null); setFrom(event.target.value); }} aria-label={tCustomer("ui.calendar.fromDate")} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-primary" /><input type="date" value={to} onChange={(event) => { setSelectedDayKey(null); setTo(event.target.value); }} aria-label={tCustomer("ui.calendar.toDate")} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-primary" /></div>{hasFilters && <button type="button" onClick={clearFilters} className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-primary hover:underline"><X size={13} /> {tCustomer("ui.actions.clearFilters")}</button>}</div>}
          </div>
          {error && <div className="m-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 sm:m-5">{error}</div>}
            <div className="hidden md:block"><div className="grid grid-cols-7 border-b border-slate-100 bg-slate-50/70">{weekdayOptions.map((day) => <div key={day} className="px-3 py-2.5 text-xs font-bold uppercase tracking-[0.12em] text-slate-400">{day}</div>)}</div><div className="grid grid-cols-7">{calendarDays.map((day) => { const key = `${day.getFullYear()}-${String(day.getMonth() + 1).padStart(2, "0")}-${String(day.getDate()).padStart(2, "0")}`; const dayGroups = bookingsByDay[key] || []; const inMonth = day.getMonth() === monthStart.getMonth() && day.getFullYear() === monthStart.getFullYear(); const hiddenCount = getHiddenItineraryGroupCount(dayGroups); return <div key={key} className={`min-h-[104px] border-b border-r border-slate-100 p-2 ${inMonth ? "bg-white" : "bg-slate-50/70"}`}><p className={`mb-1.5 text-xs font-bold ${inMonth ? "text-slate-700" : "text-slate-300"}`}>{day.getDate()}</p><div className="space-y-1">{dayGroups.slice(0, 3).map((group) => <button key={group.key} type="button" onClick={() => setSelectedDayKey(key)} className="block w-full rounded-lg border-l-2 border-primary bg-secondary px-2 py-1 text-left transition hover:bg-secondary/70 focus:outline-none focus:ring-2 focus:ring-primary/30" aria-label={tCustomer("ui.calendar.showItinerary", { activity: group.activityName })}><p className="truncate text-[11px] font-bold text-primary">{group.activityName}</p><p className="mt-0.5 text-[10px] text-primary">{bookingTime(group.slotStartsAt)} · {tCustomer("ui.calendar.paxCount", { count: group.totalQty })}</p></button>)}{hiddenCount > 0 && <button type="button" onClick={() => setSelectedDayKey(key)} aria-label={tCustomer("ui.calendar.showMore", { count: hiddenCount, date: formatDate(day, locale, { weekday: "long", day: "numeric", month: "long", year: "numeric" }) })} className="px-2 text-left text-[10px] font-bold text-primary underline-offset-2 hover:underline focus:outline-none focus:ring-2 focus:ring-primary/30">{tCustomer("ui.calendar.more", { count: hiddenCount })}</button>}</div></div>; })}</div></div><div className="space-y-3 p-4 md:hidden">{itineraryGroups.length === 0 ? <EmptyState icon={<CalendarDays size={36} />} title={tCustomer("ui.calendar.noBookingsFound")} description={hasFilters ? tCustomer("ui.calendar.changeFilters") : tCustomer("ui.calendar.afterCheckout")} /> : itineraryGroups.map((group) => <button key={group.key} type="button" onClick={() => group.slotStartsAt && setSelectedDayKey(calendarDateKey(new Date(group.slotStartsAt)))} className="block w-full rounded-2xl border border-border bg-slate-50/70 p-4 text-left transition hover:border-primary/30 focus:outline-none focus:ring-2 focus:ring-primary/30"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-slate-800">{group.activityName}</p><p className="mt-1 text-xs font-semibold text-primary">{bookingDate(group.slotStartsAt)} · {bookingTime(group.slotStartsAt)}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${statusClass(group.status)}`}>{statusLabel(group.status)}</span></div><p className="mt-3 flex items-center gap-1 text-xs text-slate-500"><MapPin size={13} /> {outletMap.get(group.outletId)?.name || tCustomer("ui.labels.mywisataOutlet")} · {tCustomer("ui.calendar.paxCount", { count: group.totalQty })} · {tCustomer("ui.calendar.bookingCount", { count: group.bookings.length })}</p><p className="mt-2 flex items-center gap-1 text-[11px] text-slate-400"><Clock3 size={12} /> {tCustomer("ui.calendar.viewItinerary")}</p></button>)}</div>
        </section>
