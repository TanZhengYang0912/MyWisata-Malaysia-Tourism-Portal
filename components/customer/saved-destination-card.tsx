      ...
      <div className="space-y-3 p-4">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {saved && <button type="button" onClick={() => void toggleSaved(destination.state)} aria-label={t("ui.wishlist.removePlace", { name: destination.state })} title={t("ui.wishlist.removePlaceShort")} className="flex h-8 w-8 items-center justify-center rounded-full border border-border text-muted-foreground transition hover:border-destructive/30 hover:bg-destructive/10 hover:text-destructive"><Trash2 size={14} aria-hidden="true" /></button>}
          </div>
        </div>
      </div>
    </article>
