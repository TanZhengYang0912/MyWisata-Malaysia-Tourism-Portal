...
export function BookingDayDrawer({
  ...
  const statusLabel = (status: BookingItineraryGroup["status"]) => t(`ui.calendar.statuses.${status}`);
  const bookingTime = (value?: string) => value
    ? new Intl.DateTimeFormat(locale, { hour: "numeric", minute: "2-digit" }).format(new Date(value))
    : t("ui.calendar.timePending");
      ...
      <aside ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="booking-day-drawer-title" className="relative z-[80] flex max-h-[min(780px,calc(100vh-2rem))] w-full max-w-3xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl sm:max-h-[calc(100vh-3rem)]">
        <header className="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-5 sm:px-6">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">{t("ui.calendar.itemsOnDay", { count: groups.length })}</p>
          </div>
          <button ref={closeButtonRef} type="button" onClick={onClose} aria-label={t("ui.calendar.closeBookingList")} className="rounded-xl p-2 text-slate-500 transition hover:bg-secondary hover:text-primary focus:outline-none focus:ring-2 focus:ring-primary/30"><X size={20} /></button>
        </header>
          ...
          <div className="space-y-3">
            {groups.map((group) => {
              ...
              return <div key={group.key} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 transition hover:border-primary/30 hover:bg-white">
                ...
                <div className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2"><span className="inline-flex items-center gap-1.5"><Clock3 size={14} className="text-primary" /> {bookingTime(group.slotStartsAt)}</span><span className="inline-flex items-center gap-1.5"><Users size={14} className="text-primary" /> {t("ui.calendar.guestsAcrossBookings", { guests: group.totalQty, bookings: group.bookings.length })}</span><span className="inline-flex items-center gap-1.5 sm:col-span-2"><MapPin size={14} className="text-primary" /> {outletName}</span></div>
