    ...
    <div className="flex bg-white rounded-xl border border-gray-200 p-1 shadow-sm">
      ...
      <details className="relative">
        ...
        <form className="absolute right-0 top-full z-20 mt-2 w-64 rounded-xl border border-gray-200 bg-white p-3 shadow-xl" onSubmit={(event) => { event.preventDefault(); if (!from) return; const params = new URLSearchParams({ filter: 'custom', from, ...(to ? { to } : {}) }); router.push(`?${params.toString()}`); }}>
          ...
          <label className="mb-2 block text-xs text-gray-600">{t('filters.from')}<input type="date" value={from} onChange={(event) => setFrom(event.target.value)} className="mt-1 h-9 w-full rounded-lg border border-gray-200 px-2 text-sm" required /></label>
          <label className="block text-xs text-gray-600">{t('filters.to')}<input type="date" value={to} min={from || undefined} onChange={(event) => setTo(event.target.value)} className="mt-1 h-9 w-full rounded-lg border border-gray-200 px-2 text-sm" /></label>
