...
export default function PriceRuleManager({ vendorId, productId, productOptions = [] }: { vendorId: string; productId: string; productOptions?: ProductOption[] }) {
  ...
  async function addRule() {
    ...
    const payload = await response.json();
    setBusy(false);
    if (!response.ok) { const message = payload.error?.message || 'Could not add pricing rule.'; setMessage(message); showFeedback('error', message); return; }
    setLabel(''); setFixedAmount(''); setMinQuantity(''); setBundleProductIds([]); setMessage('Pricing rule added and sent for review.'); showFeedback('success', 'Pricing rule added and sent for review.'); void load();
  }
