...
import type { DemoLocation, DemoPlaceWithDistance } from "@/lib/demo-map/types";
...
function stateColor(stateId: string, selected: boolean): string {
  if (selected) return "#F0B64D";
  const state = DEMO_STATES.find((item) => item.id === stateId);
  return state?.region === "Borneo" ? "#A9D9CE" : "#DCEDE6";
}

function placeColor(place: DemoPlaceWithDistance): string {
  return place.accent;
}
...
export function MalaysiaStateMap({
  ...
  const clusters = useMemo(() => {
    if (zoomedIn) return [];
    const grouped = new Map<string, DemoPlaceWithDistance[]>();
  ...
  return (
    <div className="relative overflow-hidden rounded-[2rem] border border-[#b8ddd2] bg-[#cfeee8] shadow-[0_18px_44px_rgba(19,76,68,0.12)]">
      ...
      <div className="absolute right-4 top-4 z-10 flex flex-col overflow-hidden rounded-xl border border-white/80 bg-white/90 shadow-lg backdrop-blur-sm">
        <button type="button" aria-label="Zoom in" onClick={() => setZoom((value) => Math.min(2, value + 1))} className="px-3 py-2 text-sm font-bold text-[#185f58] hover:bg-[#edf8f4]">+</button>
        <button type="button" aria-label="Zoom out" onClick={() => setZoom((value) => Math.max(1, value - 1))} className="border-t border-[#e2eee9] px-3 py-2 text-sm font-bold text-[#185f58] hover:bg-[#edf8f4]">−</button>
      </div>
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label="Interactive map of Malaysia showing all states and federal territories" className="relative block h-auto min-h-[360px] w-full">
        ...
        {DEMO_STATES.map((state) => {
          ...
          return (
            <g key={`label-${state.id}`} className="pointer-events-none">
              <text x={point.x} y={point.y} textAnchor="middle" fill="#185f58" fontSize={compact ? 10 : 11} fontWeight={700} paintOrder="stroke" stroke="#eaf8f4" strokeWidth="4" strokeLinejoin="round">
                {state.name}
              </text>
            </g>
          );
        ...
        {!zoomedIn && clusters.map((cluster) => {
          ...
          return (
            ...
            >
              <circle cx={cluster.point.x} cy={cluster.point.y} r={cluster.places.length > 1 ? 17 : 9} fill="#17665D" stroke="#ffffff" strokeWidth="4" />
              ...
              {cluster.places.length === 1 && <circle cx={cluster.point.x} cy={cluster.point.y} r={3} fill="#F5D98F" />}
            </g>
          );
        })}
        ...
        {zoomedIn && places.map((place) => {
          ...
          return (
            ...
            >
              <circle cx={point.x} cy={point.y} r={selected ? 11 : 8} fill={placeColor(place)} stroke="#ffffff" strokeWidth={selected ? 4 : 3} />
        ...
        {locationPoint && (
          <g aria-label="Your current location">
            <circle cx={locationPoint.x} cy={locationPoint.y} r="14" fill="#17665D" opacity="0.18" />
            <circle cx={locationPoint.x} cy={locationPoint.y} r="6" fill="#17665D" stroke="#ffffff" strokeWidth="3" />
          </g>
        )}
      </svg>
      <div className="absolute bottom-4 left-4 right-4 flex flex-wrap items-center justify-between gap-2 text-[10px] font-semibold text-[#185f58]">
        <span className="rounded-full border border-white/80 bg-white/85 px-3 py-1.5 backdrop-blur-sm">{zoomedIn ? "Place pins" : `${places.length} local stories · tap a cluster`}</span>
