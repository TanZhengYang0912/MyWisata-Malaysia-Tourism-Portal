...
export default function VendorOutletsPage() {
  ...
  if (managerShopMode && vendorId && outlets[0] && managerShopEditMode) return <OutletPageBuilder vendorId={vendorId} outletId={outlets[0].id} outletName={outletShortName(outlets[0].name)} outletAddress={outlets[0].address} outletCity={outlets[0].city} outletState={outlets[0].state} outletPhone={outlets[0].phone} onClose={() => router.push('/vendor/outlets?mode=shop')} />;
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="mb-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><MapPinned size={15} /> Malaysia outlet network</div><h1 className="text-2xl font-bold tracking-tight text-gray-950">Outlets</h1><p className="mt-1 text-sm text-gray-500">A clear view of every location, from Kuala Lumpur to Kota Kinabalu.</p></div>{isOwner && <button type="button" onClick={() => { setEditingOutlet(null); setShowForm(true); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-primary/90"><CirclePlus size={17} /> Add outlet</button>}</header>
      ...
      {loading ? <div className="grid gap-3 md:grid-cols-2">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="h-28 animate-pulse rounded-2xl bg-gray-100" />)}</div> : outlets.length === 0 ? <div className="rounded-2xl border border-gray-100 bg-white px-6 py-16 text-center text-gray-400 shadow-sm"><Store className="mx-auto mb-3 opacity-30" size={34} /><p className="text-sm">No outlets match these filters.</p><button type="button" onClick={clearFilters} className="mt-3 text-sm font-semibold text-primary hover:underline">Clear filters</button></div> : <>
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-gray-500"><span>{pagination.total.toLocaleString()} outlets across Malaysia</span><div className="flex flex-wrap items-center gap-3"><span>10 per page</span>{isOwner && <label className="inline-flex items-center gap-2 font-semibold text-gray-700"><input type="checkbox" aria-label="Select all outlets on current page" checked={outlets.length > 0 && outlets.every((outlet) => selectedIds.includes(outlet.id))} onChange={(event) => setSelectedIds(event.target.checked ? outlets.map((outlet) => outlet.id) : [])} /> Select page</label>}</div></div>
        <section className="grid gap-3 md:grid-cols-2">{outlets.map((outlet) => <article key={outlet.id} className="group flex min-w-0 gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-primary/20 hover:shadow-md">{isOwner && <div className="pt-1"><input type="checkbox" checked={selectedIds.includes(outlet.id)} onChange={() => toggleSelected(outlet.id)} aria-label={'Select ' + outlet.name} /></div>}<CompactThumbnail src={outlet.coverUrl} alt={outlet.name} kind="outlet" size="md" /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><button type="button" onClick={() => setSelectedOutlet(outlet)} className="block max-w-full truncate text-left font-semibold text-gray-900 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">{outletShortName(outlet.name)}</button><p className="mt-1 flex items-center gap-1 truncate text-xs text-gray-500"><MapPinned size={12} aria-hidden="true" /> {outletLocation(outlet.city, outlet.state)}</p><p className="mt-1 font-mono text-[11px] text-gray-400">{outlet.display_id || outletIdLabel(outlet.id)}</p></div><StatusBadge status={outlet.status} /></div><div className="mt-4 flex items-center justify-between gap-2 border-t border-gray-100 pt-3"><span className="text-xs text-gray-500">{outlet.productsCount > 0 ? <><strong className="text-gray-800">{outlet.productsCount}</strong> {outlet.productsCount === 1 ? 'listing' : 'listings'}</> : <span className="font-medium text-gray-600">No listings yet</span>}</span><div className="flex items-center gap-1">{outlet.productsCount === 0 && canManageOutlet && <button type="button" onClick={() => router.push('/vendor/products')} className="rounded-lg px-2.5 py-2 text-xs font-semibold text-primary hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">Manage listings</button>}<button type="button" onClick={() => setSelectedOutlet(outlet)} aria-label={`View details for ${outletShortName(outlet.name)}`} title="View details" className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-xs font-semibold text-gray-600 hover:bg-gray-100 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"><Eye size={16} aria-hidden="true" /><span>View details</span></button>{isOwner && <button type="button" onClick={() => { setEditingOutlet(outlet); setShowForm(true); }} aria-label={`Edit ${outletShortName(outlet.name)}`} title="Edit outlet" className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-xs font-semibold text-gray-600 hover:bg-gray-100 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"><Pencil size={16} aria-hidden="true" /><span>Edit outlet</span></button>}</div></div></div></article>)}</section>{pagination.totalPages > 1 && <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm"><PaginationControls page={pagination.page} totalPages={pagination.totalPages} total={pagination.total} pageSize={pagination.pageSize} onPageChange={(page) => { setPagination((current) => ({ ...current, page })); loadOutlets(page); }} /></div>}</>}
      ...
      {selectedOutlet && <div className="fixed inset-0 z-40 bg-gray-950/20" onClick={() => setSelectedOutlet(null)}><aside onClick={(event) => event.stopPropagation()} className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto overscroll-contain bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Outlet details</p><h2 className="mt-1 text-xl font-bold text-gray-950">{outletShortName(selectedOutlet.name)}</h2><p className="mt-1 text-xs text-gray-500">{outletLocation(selectedOutlet.city, selectedOutlet.state)}</p></div><button type="button" aria-label="Close outlet details" onClick={() => setSelectedOutlet(null)} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"><X size={18} aria-hidden="true" /></button></div><div className="mt-5"><CompactThumbnail src={selectedOutlet.coverUrl} alt={selectedOutlet.name} kind="outlet" size="md" /></div><div className="mt-5 space-y-3 text-sm"><div className="rounded-xl bg-gray-50 p-3"><p className="text-xs text-gray-500">Outlet ID</p><div className="mt-1 flex items-center justify-between gap-2"><p className="break-all font-mono text-xs font-semibold text-gray-900">{selectedOutlet.display_id || selectedOutlet.id}</p><button type="button" aria-label="Copy outlet ID" onClick={() => copyOutletId(selectedOutlet.display_id || selectedOutlet.id)} title="Copy outlet ID" className="shrink-0 rounded-lg p-2 text-gray-500 hover:bg-white hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"><Copy size={15} aria-hidden="true" /></button></div></div><div className="rounded-xl bg-gray-50 p-3"><p className="text-xs text-gray-500">Location</p><p className="mt-1 font-semibold text-gray-900">{outletLocation(selectedOutlet.city, selectedOutlet.state)}</p><p className="mt-1 text-gray-500">{selectedOutlet.address || 'No address added yet.'}</p></div>{isOwner && vendorId && <OutletManagerPanel vendorId={vendorId} outletId={selectedOutlet.id} manager={selectedOutlet.manager} pendingInvitation={selectedOutlet.pendingInvitation} onChanged={() => { setSelectedOutlet(null); loadOutlets(pagination.page); }} />}<div className="grid grid-cols-2 gap-3"><div className="rounded-xl bg-gray-50 p-3"><p className="text-xs text-gray-500">Listings</p><p className="mt-1 font-semibold text-gray-900">{selectedOutlet.productsCount}</p></div><div className="rounded-xl bg-gray-50 p-3"><p className="text-xs text-gray-500">Status</p><div className="mt-1"><StatusBadge status={selectedOutlet.status} /></div></div></div>{selectedOutlet.phone && <p className="text-gray-600">Phone: {selectedOutlet.phone}</p>}{selectedOutlet.email && <p className="text-gray-600">Email: {selectedOutlet.email}</p>}{selectedOutlet.lat && selectedOutlet.lng && <a className="inline-flex items-center gap-1 font-semibold text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40" href={'https://www.google.com/maps/dir/?api=1&destination=' + selectedOutlet.lat + ',' + selectedOutlet.lng} target="_blank" rel="noreferrer"><Landmark size={15} aria-hidden="true" /> Open directions <ArrowRight size={15} aria-hidden="true" /></a>}</div><div className="mt-7 flex flex-col gap-2"><button type="button" onClick={() => { setBuilderOutlet(selectedOutlet); setSelectedOutlet(null); }} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-primary/20 bg-secondary px-4 py-2.5 text-sm font-semibold text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">Edit shop page</button><div className="flex gap-2">{isOwner && <button type="button" onClick={() => { setEditingOutlet(selectedOutlet); setSelectedOutlet(null); setShowForm(true); }} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"><Pencil size={15} aria-hidden="true" /> Edit outlet</button>}{isOwner && selectedOutlet.status === 'active' && <button type="button" onClick={() => closeOutlet(selectedOutlet.id)} className="rounded-xl border border-red-200 px-4 py-2.5 text-sm font-semibold text-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300">Close</button>}</div></div></aside></div>}
      {builderOutlet && vendorId && canManageOutlet && <OutletPageBuilder vendorId={vendorId} outletId={builderOutlet.id} outletName={outletShortName(builderOutlet.name)} outletAddress={builderOutlet.address} outletCity={builderOutlet.city} outletState={builderOutlet.state} outletPhone={builderOutlet.phone} onClose={() => setBuilderOutlet(null)} />}
      <style jsx global>{`
        .fixed.inset-0.z-40 {
          display: flex;
          align-items: center;
