...
interface OutletData { id: string; name: string; slug: string; city: string | null; state: string | null; postcode?: string | null; country?: string | null; lat: number | null; lng: number | null; phone: string | null; email: string | null; status: string; address: string | null; coverUrl?: string | null; productsCount: number; }
...
export default function VendorOutletsPage() {
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="mb-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700"><MapPinned size={15} /> Malaysia outlet network</div><h1 className="text-2xl font-bold tracking-tight text-gray-950">Outlets</h1><p className="mt-1 text-sm text-gray-500">A clear view of every location, from Kuala Lumpur to Kota Kinabalu.</p></div><button type="button" onClick={() => { setEditingOutlet(null); setShowForm(true); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-800"><CirclePlus size={17} /> Add outlet</button></header>
