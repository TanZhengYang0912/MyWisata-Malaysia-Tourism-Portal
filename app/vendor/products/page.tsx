...
import { Archive, ArrowRight, Check, CirclePlus, Copy, Eye, PackageCheck, Pencil, RotateCcw, SlidersHorizontal, Utensils, X } from 'lucide-react';
...
export default function VendorProductsPage() {
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="mb-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700"><Utensils size={15} /> Catalogue management</div><h1 className="text-2xl font-bold tracking-tight text-gray-950">Products & activities</h1><p className="mt-1 text-sm text-gray-500">Keep each Malaysian listing easy to scan and ready to book.</p>{!isOwner && <div className="mt-3 inline-flex max-w-full items-center gap-2 rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-xs text-emerald-900"><span className="font-semibold uppercase tracking-[0.12em] text-emerald-700">Assigned outlet</span><span className="truncate font-semibold">{outletShortName(user?.activeOutletName || scopedOutlet?.name)}</span><span className="shrink-0 font-mono text-[10px] text-emerald-700">{outletIdLabel(user?.activeOutletIds?.[0] || scopedOutlet?.id)}</span></div>}</div>{canManageOutlet && <button type="button" onClick={() => { setEditingProduct(null); setShowForm(true); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-800"><CirclePlus size={17} /> Add listing</button>}</header>
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        {loading ? <div className="space-y-3 p-5">{Array.from({ length: 5 }).map((_, index) => <div key={index} className="h-20 animate-pulse rounded-xl bg-gray-100" />)}</div> : products.length === 0 ? <div className="px-6 py-16 text-center text-gray-400"><PackageCheck className="mx-auto mb-3 opacity-30" size={34} /><p className="text-sm">No listings match these filters.</p><button type="button" onClick={clearFilters} className="mt-3 text-sm font-semibold text-emerald-700 hover:underline">Clear filters</button></div> : <>
          ...
          <div className="divide-y divide-gray-100">{products.map((product) => <article key={product.id} className={`grid gap-3 px-4 py-4 transition hover:bg-emerald-50/30 ${tableGridClass} md:items-center md:gap-4 md:px-5`}>{isOwner && <div><input type="checkbox" checked={selectedIds.includes(product.id)} onChange={() => toggleSelected(product.id)} aria-label={'Select ' + product.name} /></div>}<div className="flex min-w-0 items-center gap-3"><CompactThumbnail src={product.cover_url} alt={product.name} kind={imageKind(product.product_type)} /><div className="min-w-0"><button type="button" onClick={() => setSelectedProduct(product)} className="block max-w-full text-left font-semibold leading-5 text-gray-900 hover:text-emerald-700 line-clamp-2">{product.name}</button>                  <p className="mt-1 truncate text-xs text-gray-500">{typeLabel(product.product_type)} {product.requires_booking ? '· Booking required' : ''}</p>
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="mt-1 group/id inline-flex items-center gap-1 font-mono text-[10px] text-gray-400 hover:text-emerald-700 transition-colors focus:outline-none"
                  >
      ...
      {selectedProduct && canManageOutlet && vendorId && <div className="fixed bottom-4 left-4 right-4 z-50 rounded-2xl bg-white p-4 shadow-2xl md:left-auto md:max-w-xl"><PriceRuleManager vendorId={vendorId} productId={selectedProduct.id} /></div>}
    </div>
  );
}
