...
import { Archive, CirclePlus, Copy, Download, Eye, PackageCheck, Pencil, RotateCcw, SlidersHorizontal, Utensils } from 'lucide-react';
...
export default function VendorProductsPage() {
  ...
  const tableGridClass = isOwner
    ? 'xl:min-w-[960px] xl:grid-cols-[32px_minmax(280px,2fr)_minmax(180px,1.2fr)_130px_110px_140px]'
    : 'xl:min-w-[960px] xl:grid-cols-[minmax(280px,2fr)_minmax(180px,1.2fr)_130px_110px_140px]';
  const filterSelects = [
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-1.5 border-b border-gray-200 pb-2">
        {[
          { key: '', label: t('ui.products.allStatuses') },
          { key: 'active', label: t('ui.status.active') },
          { key: 'archived', label: t('ui.status.archived') },
        ].map((tab) => {
          ...
          return (
            <button
              key={tab.key}
              type="button"
              onClick={() => setFilters((current) => ({ ...current, status: tab.key }))}
            ...
            <div className="divide-y divide-gray-100">
              {products.map((product) => (
                <article key={product.id} className={`grid gap-3 px-4 py-4 transition hover:bg-secondary/30 ${tableGridClass} xl:items-center xl:gap-4 xl:px-5`}>
                  ...
                  {/* LISTING column: Thumbnail + Name + Type/Booking + Product ID with copy button */}
                  <div className="flex min-w-0 items-center gap-3">
                    <CompactThumbnail src={productImageUrl(product.cover_url)} alt={product.name} kind={imageKind(product.product_type)} />
                    <div className="min-w-0">
                      <button type="button" onClick={() => setSelectedProduct(product)} className="block max-w-full text-left font-semibold leading-5 text-gray-900 hover:text-primary line-clamp-2">
                        {product.name}
                      </button>
                  ...
                  {/* OUTLET column: Outlet name, city/state, outlet ID */}
                  <div className="text-xs text-gray-500">
                    <span className="block truncate font-medium text-gray-700">{outletShortName(product.outlet?.name || user?.activeOutletName)}</span>
                    <span className="mt-0.5 block truncate">{outletLocation(product.outlet?.city, product.outlet?.state)}</span>
                  ...
                  {/* ACTION column: View details, Share, Edit, Archive/Restore */}
