...
export default function VendorProductsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      /> : <>
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="mb-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><Utensils size={15} /> {t('ui.products.catalogueManagement')}</div><h1 className="text-2xl font-bold tracking-tight text-gray-950">{t('ui.products.title')}</h1><p className="mt-1 text-sm text-gray-500">{t('ui.products.description')}</p>{!isOwner && <div className="mt-3 inline-flex max-w-full items-center gap-2 rounded-xl border border-primary/10 bg-secondary px-3 py-2 text-xs text-primary"><span className="font-semibold uppercase tracking-[0.12em] text-primary">{t('ui.products.assignedOutlet')}</span><span className="truncate font-semibold">{outletShortName(user?.activeOutletName || scopedOutlet?.name)}</span><span className="shrink-0 font-mono text-[10px] text-primary">{outletIdLabel(user?.activeOutletIds?.[0] || scopedOutlet?.id)}</span></div>}</div>{canManageOutlet && <button type="button" onClick={() => { setEditingProduct(null); setShowForm(true); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-primary/90"><CirclePlus size={17} /> {t('ui.products.addListing')}</button>}</header>
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        {loading ? <div className="space-y-3 p-5">{Array.from({ length: 5 }).map((_, index) => <div key={index} className="h-20 animate-pulse rounded-xl bg-gray-100" />)}</div> : products.length === 0 ? <div className="px-6 py-16 text-center text-gray-400"><PackageCheck className="mx-auto mb-3 opacity-30" size={34} /><p className="text-sm">{t('ui.products.noMatches')}</p><button type="button" onClick={clearFilters} className="mt-3 text-sm font-semibold text-primary hover:underline">{t('ui.common.clearFilters')}</button></div> : <>
          ...
          <div className="divide-y divide-gray-100">{products.map((product) => <article key={product.id} className={`grid gap-3 px-4 py-4 transition hover:bg-secondary/30 ${tableGridClass} md:items-center md:gap-4 md:px-5`}>{isOwner && <div><input type="checkbox" checked={selectedIds.includes(product.id)} onChange={() => toggleSelected(product.id)} aria-label={t('ui.products.selectListing', { name: product.name })} /></div>}<div className="flex min-w-0 items-center gap-3"><CompactThumbnail src={product.cover_url} alt={product.name} kind={imageKind(product.product_type)} /><div className="min-w-0"><button type="button" onClick={() => setSelectedProduct(product)} className="block max-w-full text-left font-semibold leading-5 text-gray-900 hover:text-primary line-clamp-2">{product.name}</button>                  <p className="mt-1 truncate text-xs text-gray-500">{typeLabel(product.product_type)} {product.requires_booking ? t('ui.products.bookingRequired') : ''}</p>
                  ...
                  </button></div></div><div className="pl-[4.25rem] text-xs text-gray-500 md:pl-0"><span className="block truncate font-medium text-gray-700">{outletShortName(product.outlet?.name || user?.activeOutletName)}</span><span className="mt-1 block truncate">{outletLocation(product.outlet?.city, product.outlet?.state)}</span><span className="mt-1 block font-mono text-[10px] text-gray-400">{outletIdLabel(product.outlet?.id || product.outlet_id || user?.activeOutletIds?.[0])}</span></div><div className="pl-[4.25rem] text-sm font-semibold text-gray-900 md:pl-0">{toRM(Number(product.base_price))}<span className={`mt-1 block text-[11px] font-semibold ${product.requires_booking ? 'text-gray-400' : product.availableStock === 0 ? 'text-red-600' : product.availableStock !== undefined && product.availableStock <= (product.lowStockThreshold ?? 5) ? 'text-amber-700' : 'text-primary'}`}>{product.requires_booking ? t('ui.products.timeSlots') : product.availableStock === 0 ? t('ui.products.outOfStock') : t('ui.products.stockCount', { count: product.availableStock, low: product.availableStock !== undefined && product.availableStock <= (product.lowStockThreshold ?? 5) ? t('ui.products.lowStockSuffix') : '' })}</span></div><div className="pl-[4.25rem] md:pl-0"><StatusBadge status={product.status} /></div><div className="flex items-center justify-end gap-1 border-t border-gray-100 pt-3 md:border-0 md:pt-0"><button type="button" onClick={() => setSelectedProduct(product)} title={t('ui.products.viewDetails')} className="rounded-lg p-2 text-gray-500 hover:bg-gray-100 hover:text-primary"><Eye size={16} /></button><ShareButton compact shareType="product" contentId={product.id} title={product.name} plainOnly />{canManageOutlet && <><button type="button" onClick={() => { setEditingProduct(product); setShowForm(true); }} title={t('ui.products.editListing')} className="rounded-lg p-2 text-gray-500 hover:bg-secondary hover:text-primary"><Pencil size={16} /></button>{product.status === 'archived' ? <button type="button" onClick={() => handleRestore(product.id)} title={t('ui.products.restoreListing')} className="rounded-lg p-2 text-primary hover:bg-secondary"><RotateCcw size={16} /></button> : <button type="button" onClick={() => handleDelete(product.id)} title={t('ui.products.archiveListing')} className="rounded-lg p-2 text-red-500 hover:bg-red-50"><Archive size={16} /></button>}</>}</div></article>)}</div><PaginationControls page={pagination.page} totalPages={pagination.totalPages} total={pagination.total} pageSize={pagination.pageSize} onPageChange={(page) => { setPagination((current) => ({ ...current, page })); loadProducts(page); }} /></>}
      </section>
      </>}
