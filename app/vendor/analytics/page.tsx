...
export default async function VendorAnalyticsPage() {
  ...
  if (!data) return <div className="rounded-2xl bg-white p-10 text-center text-gray-500">No approved vendor is linked to this account.</div>;

  return (
    <div className="space-y-8">
      <div><div className="mb-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700"><BarChart3 size={15} /> Malaysia-wide performance</div><h1 className="text-3xl font-bold tracking-tight text-gray-950">Analytics</h1><p className="mt-1 text-sm text-gray-500">A twelve-month view of your outlets, products and traveller demand.</p></div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3"><div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm"><Activity className="mb-4 text-emerald-700" size={20} /><p className="text-2xl font-bold text-gray-950">{formatRM(data.stats.totalRevenue)}</p><p className="mt-1 text-sm text-gray-500">Revenue in selected period</p></div><div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm"><ShoppingBag className="mb-4 text-sky-700" size={20} /><p className="text-2xl font-bold text-gray-950">{data.stats.totalOrders.toLocaleString()}</p><p className="mt-1 text-sm text-gray-500">Paid orders</p></div><div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm"><MapPinned className="mb-4 text-amber-700" size={20} /><p className="text-2xl font-bold text-gray-950">{data.stats.activeOutlets}</p><p className="mt-1 text-sm text-gray-500">Active outlets across Malaysia</p></div></div>
      ...
      <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-sm text-emerald-900"><div className="flex items-start gap-3"><CalendarDays className="mt-0.5 shrink-0" size={18} /><p><strong>Reading this view:</strong> the twelve-month chart is calculated from paid and completed order items in Supabase. Use the dashboard period controls for shorter operating windows.</p><ArrowUpRight className="ml-auto shrink-0" size={18} /></div></div>
    </div>
  );
}
