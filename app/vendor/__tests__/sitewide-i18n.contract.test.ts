...
export const VENDOR_I18N_FILES = [
  ...
  'components/vendor/outlet-action-group.tsx',
...
const priorTaskFiles = new Set([
  ...
  'components/vendor/redemption-scanner.tsx',
  ...
  'components/shared/currency-switcher.tsx',
...
function actualRenderedInventory(): string[] {
  ...
  const visit = (directory: string) => {
    for (const entry of readdirSync(resolve(process.cwd(), directory), {
      withFileTypes: true,
    })) {
...
function hasTranslationBinding(source: string): boolean {
  return /useTranslation\(["'](?:vendor|common)["']\)|getServerTranslation\(["'](?:vendor|common)["']\)/.test(
    source,
  );
}
...
function callsBoundTranslator(source: string): boolean {
  const bindings = [
    ...source.matchAll(
      /\{\s*t(?:\s*:\s*([A-Za-z_$][\w$]*))?[^}]*\}\s*=\s*(?:await\s+)?(?:useTranslation|getServerTranslation)\(/g,
    ),
  ].map((match) => match[1] ?? 't');
...
describe('vendor and outlet sitewide i18n contract', () => {
  it('reconciles the exact Task 8 list with the rendered source inventory', () => {
    ...
    for (const file of VENDOR_I18N_FILES) {
      expect(
        actual.has(file),
        `Task 8 file is missing from the rendered inventory: ${file}`,
      ).toBe(true);
      expect(() => read(file), file).not.toThrow();
    }
    for (const file of actual) {
      expect(
        listed.has(file) || priorTaskFiles.has(file),
        `Unclassified vendor source file: ${file}`,
      ).toBe(true);
    }
  });
  ...
  it.each(VENDOR_I18N_FILES)('uses a live translation call in %s', (file) => {
    ...
    expect(
      hasTranslationBinding(source),
      `${file} must bind vendor/common translations`,
    ).toBe(true);
    expect(
      callsBoundTranslator(source),
      `${file} must call its bound translator; a hook alone is insufficient`,
    ).toBe(true);
  });
  ...
  it('keeps representative vendor semantics dedicated and complete in all locales', () => {
    ...
    for (const [locale, values] of Object.entries(expected)) {
      for (const [key, value] of Object.entries(values)) {
        expect(
          resourceValue(resources[locale as keyof typeof resources], key),
          `${locale}:${key}`,
        ).toBe(value);
      }
    }
  ...
  it('covers high-risk vendor clusters with the vendor namespace', () => {
    for (const file of [
      ...
      expect(read(file), file).toMatch(
        /(?:useTranslation|getServerTranslation)\(["']vendor["']/,
      );
    }
  });
  ...
  it('translates route-owned high-risk controls instead of only binding a hook', () => {
    const expectations: Record<string, { keys: string[]; oldCopy: string[] }> =
      {
        'app/vendor/bookings/page.tsx': {
          keys: [
            'ui.bookings.searchPlaceholder',
            'ui.bookings.selectCurrentPage',
            'ui.bookings.reservationDetails',
          ],
          oldCopy: [
            'Search booking ID, guest or experience',
            'Select current page',
            'Reservation details',
          ],
        },
        'app/vendor/inbox/page.tsx': {
          keys: [
            'ui.inbox.traveller',
            'ui.inbox.noMessages',
            'ui.inbox.lastMessage',
          ],
          oldCopy: ['No messages yet', 'Your last message'],
        },
        'app/vendor/orders/page.tsx': {
          keys: [
            'ui.orders.customerColumn',
            'ui.orders.selectCurrentPage',
            'ui.orders.orderDetails',
          ],
          oldCopy: [
            'Select current page',
            'Order details',
        ...
        'app/vendor/outlets/page.tsx': {
          keys: [
            'ui.outlets.searchPlaceholder',
            'ui.outlets.selectCurrentPage',
            'ui.outlets.outletDetails',
          ],
          oldCopy: [
            'Search outlet ID, name or city',
            'Select current page',
            'Outlet details',
          ],
        },
        'app/vendor/products/page.tsx': {
          keys: [
            'ui.products.searchPlaceholder',
            'ui.products.selectCurrentPage',
            'ui.products.noMatches',
          ],
          oldCopy: [
            'Search product ID, name or slug',
            'Select current page',
        ...
        'app/vendor/profile/page.tsx': {
          keys: [
            'ui.profile.unsavedChanges',
            'ui.profile.businessIdentity',
            'ui.profile.livePreview',
          ],
          oldCopy: [
            'Unsaved changes',
            'These details appear on your public vendor profile',
            'Live preview',
          ],
        },
        'app/vendor/vouchers/page.tsx': {
          keys: [
            'ui.vouchers.uploadCsv',
            'ui.vouchers.performance',
            'ui.vouchers.closeDetails',
          ],
          oldCopy: [
            'Upload voucher CSV',
            'Voucher performance',
        ...
        'app/vendor/wallet/page.tsx': {
          keys: [
            'ui.wallet.summaryLabel',
            'ui.wallet.transactionHistory',
            'ui.wallet.submitRequest',
          ],
    ...
    for (const [file, expectation] of Object.entries(expectations)) {
      ...
      for (const key of expectation.keys) {
        expect(source, `${file} must call ${key}`).toMatch(
          new RegExp(`t\\(["']${key}["']\\)`),
        );
      }
      for (const oldCopy of expectation.oldCopy) {
        expect(
          source,
          `${file} still contains fixed English: ${oldCopy}`,
  ...
  it('removes confirmed fixed English fallbacks from corrected vendor surfaces', () => {
    const expectations: Record<string, { keys: string[]; oldCopy: string[] }> =
      {
        'app/vendor/products/page.tsx': {
          keys: [
            'ui.products.loadFailed',
            'ui.products.archiveConfirm',
            'ui.products.archiveFailed',
            'ui.products.archived',
            'ui.products.batchSummarySkipped',
            'ui.products.batchSummary',
          ],
          oldCopy: [
        ...
        'app/vendor/profile/page.tsx': {
          keys: [
            ...
            'ui.profile.aiUnavailable',
            'ui.profile.saved',
            'ui.profile.saveFailed',
            'ui.profile.profileLinkCopied',
          ],
          oldCopy: [
        ...
        'components/vendor/product-form.tsx': {
          keys: [
            'productForm.categoryInvalid',
            'productForm.capacityPlaceholder',
            'productForm.galleryImageAlt',
            'productForm.removeGalleryImage',
          ],
          oldCopy: [
    ...
    const requiredKeys = Object.values(expectations).flatMap(
      ({ keys }) => keys,
    );

    for (const [locale, resource] of Object.entries(resources)) {
      for (const key of requiredKeys) {
        expect(resourceValue(resource, key), `${locale}:${key}`).toEqual(
          expect.any(String),
        );
      }
    }
