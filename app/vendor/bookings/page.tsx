...
export default function VendorBookingsPage() {
  ...
  async function saveSchedule() {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error?.message || 'Could not save operating hours');
      const persistedOperatingHours = payload.data?.operating_hours as OperatingHours | undefined;
      if (!persistedOperatingHours) throw new Error('Supabase did not return the saved operating hours');
      ...
      setSaveMessage('Operating hours saved to Supabase.');
      showFeedback('success', 'Operating hours saved to Supabase.');
    } catch (requestError) {
      const message = requestError instanceof Error ? requestError.message : 'Could not save operating hours';
      setError(message);
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div><div className="mb-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><CalendarDays size={15} /> Booking operations</div><h1 className="text-2xl font-bold tracking-tight text-gray-950">Bookings</h1><p className="mt-1 max-w-2xl text-sm text-gray-500">Manage reservations and operating hours by outlet, with booking slots kept separate.</p></div>
