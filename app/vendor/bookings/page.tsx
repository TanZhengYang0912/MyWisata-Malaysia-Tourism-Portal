  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div><div className="mb-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700"><CalendarDays size={15} /> Booking operations</div><h1 className="text-2xl font-bold tracking-tight text-gray-950">Bookings</h1><p className="mt-1 max-w-2xl text-sm text-gray-500">Manage reservations by outlet and keep every opening hour visible in one weekly view.</p></div>
        <button type="button" onClick={() => setShowSlotForm(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-800"><CirclePlus size={17} /> Add date slot</button>
      </header>

      <div className="grid gap-3 sm:grid-cols-4"><div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm"><p className="text-xs text-gray-500">Confirmed</p><p className="mt-1 text-2xl font-bold text-gray-950">{stats.confirmed || 0}</p><p className="mt-1 text-xs text-gray-400">Current filter</p></div><div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm"><p className="text-xs text-gray-500">Checked in</p><p className="mt-1 text-2xl font-bold text-emerald-700">{stats.checked_in || 0}</p><p className="mt-1 text-xs text-gray-400">Current filter</p></div><div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm"><p className="text-xs text-gray-500">Cancelled</p><p className="mt-1 text-2xl font-bold text-gray-950">{stats.cancelled || 0}</p><p className="mt-1 text-xs text-gray-400">Current filter</p></div><div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 shadow-sm"><p className="text-xs text-emerald-800">Outlets managed</p><p className="mt-1 text-2xl font-bold text-emerald-900">{outlets.length}</p><p className="mt-1 text-xs text-emerald-700">Choose an outlet below</p></div></div>

      <div className="rounded-2xl border border-gray-100 bg-white p-3 shadow-sm"><div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div className="flex rounded-xl bg-gray-100 p-1"><button type="button" onClick={() => switchTab('reservations')} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold ${tab === 'reservations' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}><Users size={15} /> Reservations</button><button type="button" onClick={() => switchTab('availability')} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold ${tab === 'availability' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}><Clock3 size={15} /> Availability</button></div><div className="grid flex-1 gap-2 sm:grid-cols-2 xl:ml-4 xl:flex"><label className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={15} /><input value={filters.q} onChange={(event) => setFilters((current) => ({ ...current, q: event.target.value }))} placeholder={tab === 'reservations' ? 'Search booking ID, guest or experience' : 'Search experience or outlet'} className="h-10 w-full rounded-xl border border-gray-200 pl-9 pr-3 text-sm outline-none focus:border-emerald-500" /></label><select value={filters.outletId} onChange={(event) => setOutletFilter(event.target.value)} className="h-10 rounded-xl border border-gray-200 px-3 text-sm text-gray-600 outline-none focus:border-emerald-500 xl:w-52"><option value="">All outlets</option>{outlets.map((outlet) => <option key={outlet.id} value={outlet.id}>{outletShortName(outlet.name)} · {outletLocation(outlet.city, outlet.state)}</option>)}</select><select value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value }))} className="h-10 rounded-xl border border-gray-200 px-3 text-sm text-gray-600 outline-none focus:border-emerald-500 xl:w-36"><option value="">All statuses</option>{(tab === 'reservations' ? ['confirmed', 'checked_in', 'no_show', 'cancelled'] : ['available', 'full', 'cancelled', 'expired']).map((status) => <option key={status} value={status}>{status.replace('_', ' ')}</option>)}</select></div></div><div className="mt-2 flex flex-wrap items-center gap-2 border-t border-gray-100 pt-3"><SlidersHorizontal size={15} className="text-gray-400" /><select value={filters.productId} onChange={(event) => setFilters((current) => ({ ...current, productId: event.target.value }))} className="h-9 rounded-lg border border-gray-200 px-3 text-xs text-gray-600 outline-none focus:border-emerald-500"><option value="">All experiences</option>{filteredProducts.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select><input type="date" value={filters.from} onChange={(event) => setFilters((current) => ({ ...current, from: event.target.value }))} className="h-9 rounded-lg border border-gray-200 px-3 text-xs text-gray-600" /><span className="text-xs text-gray-400">to</span><input type="date" value={filters.to} onChange={(event) => setFilters((current) => ({ ...current, to: event.target.value }))} className="h-9 rounded-lg border border-gray-200 px-3 text-xs text-gray-600" />{(filters.q || filters.status || filters.from || filters.to || filters.outletId || filters.productId) && <button type="button" onClick={clearFilters} className="text-xs font-semibold text-emerald-700 hover:underline">Clear filters</button>}</div></div>
