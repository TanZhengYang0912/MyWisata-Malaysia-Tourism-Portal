...
function dateLabel(value: string | undefined, locale: string, fallback: string) { return value ? new Intl.DateTimeFormat(locale, { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(value)) : fallback; }
function outletLabel(outlet: Outlet | null | undefined, fallback: string) { return outlet ? `${outletShortName(outlet.name)} · ${outletLocation(outlet.city, outlet.state)}` : fallback; }
function scheduleFor(outlet?: Outlet | null): Record<DayKey, DaySchedule> {
  return dayLabels.reduce((result, day) => {
    result[day] = { open: outlet?.operating_hours?.[day]?.open || '09:00', close: outlet?.operating_hours?.[day]?.close || '18:00', closed: Boolean(outlet?.operating_hours?.[day]?.closed), note: outlet?.operating_hours?.[day]?.note || '' };
    return result;
...
export default function VendorBookingsPage() {
  ...
  const loadData = useCallback(async (requestedPage = 1) => {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error?.message || t('ui.bookings.loadFailed'));
  ...
  async function checkIn(bookingId: string) {
    ...
    try {
      ...
      if (!response.ok) { const payload = await response.json(); const message = payload.error?.message || t('ui.bookings.checkInFailed'); setError(message); showFeedback('error', message); return; }
      showFeedback('success', t('ui.bookings.checkedIn'));
      setSelectedBooking(null); loadData(pagination.page);
    } catch { setError(t('ui.bookings.checkInFailed')); showFeedback('error', t('ui.bookings.checkInTryAgain')); }
  }
  async function cancelSlot(slotId: string) {
    if (!vendorId || !confirm(t('ui.bookings.cancelSlotConfirm'))) return;
    try {
      const response = await fetch(`/api/vendors/${vendorId}/slots/${slotId}`, { method: 'DELETE' });
      if (!response.ok) { const payload = await response.json(); const message = payload.error?.message || t('ui.bookings.cancelSlotFailed'); setError(message); showFeedback('error', message); return; }
      showFeedback('success', t('ui.bookings.slotCancelled'));
      loadData(pagination.page);
    } catch { setError(t('ui.bookings.cancelSlotFailed')); showFeedback('error', t('ui.bookings.cancelSlotTryAgain')); }
  }
  async function saveSchedule() {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error?.message || t('ui.bookings.saveHoursFailed'));
      const persistedOperatingHours = payload.data?.operating_hours as OperatingHours | undefined;
      if (!persistedOperatingHours) throw new Error(t('ui.bookings.savedHoursMissing'));
      setOutlets((current) => current.map((outlet) => outlet.id === selectedScheduleOutlet.id ? { ...outlet, operating_hours: persistedOperatingHours } : outlet));
      setSaveMessage(t('ui.bookings.hoursSaved'));
      showFeedback('success', t('ui.bookings.hoursSaved'));
    } catch (requestError) {
      const message = requestError instanceof Error ? requestError.message : t('ui.bookings.saveHoursFailed');
      setError(message);
  ...
  async function applyBatch(action: string) {
    ...
    setBatchMessage(payload.data?.skipped ? t('ui.bookings.batchSummarySkipped', { updated: payload.data.updated, skipped: payload.data.skipped }) : t('ui.bookings.batchSummary', { count: payload.data?.updated || 0, entity: t(`ui.bookings.entities.${entity}`) }));
  ...
  function setOutletFilter(outletId: string) { setFilters((current) => ({ ...current, outletId, productId: '' })); if (tab === 'operating-hours' && outletId) setScheduleOutletId(outletId); }
...
function OperatingHoursPanel({ metadataLoading, outlets, products, selectedOutlet, scheduleDraft, setScheduleDraft, exceptions, setExceptions, saveSchedule, savingSchedule, saveMessage, setScheduleOutletId, slots, loading, selectedIds, setSelectedIds, toggleSelected, cancelSlot, pagination, loadData }: OperatingHoursPanelProps) {
  const { t, i18n } = useTranslation('vendor');
  const locale = i18n.resolvedLanguage || i18n.language;
  return <div className="grid gap-5 xl:grid-cols-[250px_minmax(0,1fr)]"><aside className="rounded-2xl border border-gray-100 bg-white p-3 shadow-sm"><div className="flex items-center justify-between px-2 pb-3"><div><p className="text-sm font-semibold text-gray-900">{t('ui.bookings.yourOutlets')}</p><p className="text-xs text-gray-500">{t('ui.bookings.selectToEdit')}</p></div><MapPin size={17} className="text-primary" /></div><div className="space-y-1">{metadataLoading ? <div className="space-y-2 p-2">{[1, 2, 3].map((item) => <div key={item} className="h-12 animate-pulse rounded-xl bg-gray-100" />)}</div> : outlets.map((outlet) => <button type="button" key={outlet.id} onClick={() => setScheduleOutletId(outlet.id)} className={`flex w-full items-center justify-between rounded-xl px-3 py-3 text-left transition ${selectedOutlet?.id === outlet.id ? 'bg-secondary text-primary ring-1 ring-primary/20' : 'text-gray-600 hover:bg-gray-50'}`}><span className="min-w-0"><span className="block truncate text-sm font-semibold">{outlet.name}</span><span className="mt-1 block truncate text-xs text-gray-500">{outlet.city || outlet.state || t('ui.bookings.malaysia')}</span></span><ChevronRight size={15} className="shrink-0" /></button>)}</div></aside><div className="space-y-5"><div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 border-b border-gray-100 pb-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">{t('ui.bookings.weeklyHours')}</p><h2 className="mt-1 text-xl font-bold text-gray-950">{outletLabel(selectedOutlet, t('ui.bookings.allOutlets'))}</h2><p className="mt-1 text-sm text-gray-500">{t('ui.bookings.weeklyHoursDescription')}</p></div><button type="button" onClick={saveSchedule} disabled={savingSchedule || !selectedOutlet} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary/90 disabled:opacity-50"><Save size={15} /> {savingSchedule ? t('ui.bookings.saving') : t('ui.bookings.saveHours')}</button></div><div className="mt-4 space-y-2">{dayLabels.map((day) => { const value = scheduleDraft[day] || {}; return <div key={day} className="grid gap-3 rounded-xl border border-gray-100 bg-gray-50/60 p-3 sm:grid-cols-[140px_100px_100px_1fr_auto] sm:items-center"><span className="text-sm font-semibold text-gray-800">{t(`ui.bookings.days.${day}`)}</span><label className="flex items-center gap-2 text-xs text-gray-500"><input type="checkbox" checked={!value.closed} onChange={(event) => setScheduleDraft((current) => ({ ...current, [day]: { ...current[day], closed: !event.target.checked } }))} /> {t('ui.bookings.open')}</label><input type="time" disabled={value.closed} value={value.open || '09:00'} onChange={(event) => setScheduleDraft((current) => ({ ...current, [day]: { ...current[day], open: event.target.value } }))} className="h-9 rounded-lg border border-gray-200 bg-white px-2 text-sm disabled:opacity-40" /><input type="time" disabled={value.closed} value={value.close || '18:00'} onChange={(event) => setScheduleDraft((current) => ({ ...current, [day]: { ...current[day], close: event.target.value } }))} className="h-9 rounded-lg border border-gray-200 bg-white px-2 text-sm disabled:opacity-40" /><span className={`text-xs font-semibold ${value.closed ? 'text-gray-400' : 'text-primary'}`}>{value.closed ? t('ui.bookings.closed') : `${value.open || '09:00'} – ${value.close || '18:00'}`}</span></div>; })}</div>{saveMessage && <p className="mt-3 text-xs font-semibold text-primary">{saveMessage}</p>}</div><div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">{t('ui.bookings.dateExceptions')}</p><h2 className="mt-1 text-lg font-bold text-gray-950">{t('ui.bookings.holidayHours')}</h2><p className="mt-1 text-sm text-gray-500">{t('ui.bookings.exceptionsDescription')}</p></div><button type="button" onClick={() => setExceptions((current) => [...current, { date: '', open: '09:00', close: '18:00', closed: true, note: '' }])} className="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50"><CirclePlus size={14} /> {t('ui.bookings.addException')}</button></div><div className="mt-4 space-y-2">{exceptions.map((exception, index) => <div key={`${exception.date}-${index}`} className="grid gap-2 rounded-xl border border-gray-100 bg-gray-50/60 p-3 sm:grid-cols-[145px_100px_100px_1fr_auto] sm:items-center"><input type="date" value={exception.date} onChange={(event) => setExceptions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, date: event.target.value } : item))} className="h-9 rounded-lg border border-gray-200 bg-white px-2 text-sm" /><label className="flex items-center gap-2 text-xs text-gray-500"><input type="checkbox" checked={!exception.closed} onChange={(event) => setExceptions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, closed: !event.target.checked } : item))} /> {t('ui.bookings.open')}</label><input type="time" disabled={exception.closed} value={exception.open || '09:00'} onChange={(event) => setExceptions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, open: event.target.value } : item))} className="h-9 rounded-lg border border-gray-200 bg-white px-2 text-sm disabled:opacity-40" /><input type="time" disabled={exception.closed} value={exception.close || '18:00'} onChange={(event) => setExceptions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, close: event.target.value } : item))} className="h-9 rounded-lg border border-gray-200 bg-white px-2 text-sm disabled:opacity-40" /><button type="button" onClick={() => setExceptions((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="rounded-lg p-2 text-gray-400 hover:bg-red-50 hover:text-red-600"><X size={15} /></button></div>)}{!exceptions.length && <p className="rounded-xl bg-gray-50 px-4 py-5 text-center text-sm text-gray-400">{t('ui.bookings.noExceptions')}</p>}</div></div><div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-gray-100 bg-gray-50/70 px-5 py-3"><label className="inline-flex items-center gap-2 text-xs font-semibold text-gray-600"><input type="checkbox" checked={slots.length > 0 && slots.every((slot) => selectedIds.includes(slot.id))} onChange={(event) => setSelectedIds(event.target.checked ? slots.map((slot) => slot.id) : [])} /> {t('ui.bookings.selectSlots')}</label><span className="text-xs text-gray-400">{t('ui.bookings.slotsSummary', { slots: pagination.total.toLocaleString(), products: products.length })}</span></div><div className="divide-y divide-gray-100">{loading ? <div className="space-y-3 p-5">{[1, 2, 3].map((item) => <div key={item} className="h-16 animate-pulse rounded-xl bg-gray-100" />)}</div> : slots.map((slot) => <article key={slot.id} className="grid gap-3 px-4 py-4 md:grid-cols-[32px_minmax(190px,1.5fr)_160px_140px_100px] md:items-center"><input type="checkbox" checked={selectedIds.includes(slot.id)} onChange={() => toggleSelected(slot.id)} aria-label={t('ui.bookings.selectSlot', { id: slot.id.slice(0, 8) })} /><div className="min-w-0"><p className="truncate font-semibold text-gray-900">{slot.products?.name || t('ui.bookings.experience')}</p><p className="mt-1 text-xs text-gray-500">{t('strictMigration.bookingReference', { date: dateLabel(slot.starts_at, locale, t('ui.bookings.noDate')), id: slot.id.slice(0, 8) })}</p></div><div className="text-xs text-gray-600"><p className="font-medium text-gray-800">{slot.outlets?.name || t('ui.bookings.outlet')}</p><p className="mt-1 text-gray-400">{slot.outlets?.city || slot.outlets?.state || ''}</p></div><div><p className="text-sm font-semibold text-gray-900">{slot.booked}/{slot.capacity}</p><StatusBadge status={slot.status} /></div><div className="flex justify-end">{slot.booked === 0 && slot.status === 'available' && <button type="button" onClick={() => cancelSlot(slot.id)} className="text-xs font-semibold text-red-600 hover:underline">{t('ui.bookings.cancel')}</button>}</div></article>)}{!slots.length && !loading && <div className="px-6 py-16 text-center text-sm text-gray-400">{t('ui.bookings.noSlots')}</div>}</div><PaginationControls page={pagination.page} totalPages={pagination.totalPages} total={pagination.total} pageSize={pagination.pageSize} onPageChange={(page) => loadData(page)} /></div></div></div>;
}
