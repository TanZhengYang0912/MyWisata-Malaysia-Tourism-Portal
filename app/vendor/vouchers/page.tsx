    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-amber-100 bg-amber-50/50 p-4">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
          ...
          <div className="rounded-xl bg-white px-3 py-2"><p className="text-[11px] text-gray-500">{t('ui.vouchers.salesGenerated')}</p><p className="mt-1 font-bold text-gray-900">{formatMYR(analyticsTotals.revenue)}</p></div>
          <div className="rounded-xl bg-white px-3 py-2"><p className="text-[11px] text-gray-500">{t('ui.vouchers.discountCost')}</p><p className="mt-1 font-bold text-gray-900">{formatMYR(analyticsTotals.discount)}</p></div>
          <div className="rounded-xl bg-white px-3 py-2"><p className="text-[11px] text-gray-500">{t('ui.vouchers.netRevenueImpact')}</p><p className="mt-1 font-bold text-primary">{formatMYR(analyticsTotals.revenueImpact)}</p></div>
        </div>
        <div className="mt-3 grid gap-2 md:grid-cols-2">{analytics.slice(0, 6).map((item, index) => <div key={item.voucherId} className="rounded-xl border border-amber-100 bg-white px-3 py-3 text-xs"><div className="flex justify-between gap-3"><span className="font-mono font-bold text-amber-700">{index === 0 ? t('ui.vouchers.bestPerformingPrefix') : ''}{item.code}</span><span className="font-semibold text-gray-800">{t('ui.vouchers.usesCount', { count: item.redemptions })}</span></div><p className="mt-1 text-gray-500">{item.name} · {item.outletName} · {item.redemptionRate === null ? t('ui.vouchers.unlimitedUsage') : t('ui.vouchers.usageCap', { percent: item.redemptionRate })} · {t('ui.vouchers.analyticsAmounts', { revenue: formatMYRNumber(item.revenue), discount: formatMYRNumber(item.discount), impact: formatMYRNumber(item.revenueImpact) })}</p></div>)}{analytics.length === 0 && <p className="text-sm text-gray-500">{t('ui.vouchers.noAnalyticsMatches')}</p>}</div>
      </section>
      ...
      {selectedVoucher && <div className="fixed inset-0 z-40 bg-gray-950/20" onClick={() => setSelectedVoucher(null)}><aside onClick={(event) => event.stopPropagation()} className="absolute right-0 top-0 h-full w-full max-w-2xl overflow-y-auto bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">{t('ui.vouchers.voucherDetails')}</p><h2 className="mt-1 text-xl font-bold text-gray-950">{selectedVoucher.name}</h2></div><button type="button" onClick={() => setSelectedVoucher(null)} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100" aria-label={t('ui.vouchers.closeDetails')}><X size={18} /></button></div><div className="mt-6"><VoucherTicket offer={{ brandName: selectedVoucher.outlets?.name || t('ui.vouchers.allOutlets'), name: selectedVoucher.name, code: selectedVoucher.code, codeLabel: t('ui.vouchers.code'), discountLabel: discountLabel(selectedVoucher), minSpendLabel: t('ui.vouchers.minimumSpend', { amount: formatMYRNumber(Number(selectedVoucher.min_spend)) }), expiryLabel: t('ui.vouchers.validRange', { from: dateLabel(selectedVoucher.valid_from, locale, t('ui.vouchers.noEndDate')), until: dateLabel(selectedVoucher.valid_until, locale, t('ui.vouchers.noEndDate')) }), availabilityLabel: `${selectedVoucher.uses_count} ${selectedVoucher.max_uses ? `/ ${selectedVoucher.max_uses}` : t('ui.vouchers.uses')}`, scopeLabel: t('ui.vouchers.outletValue', { outlet: selectedVoucher.outlets?.name || t('ui.vouchers.allOutlets') }), identityLabel: t('ui.vouchers.campaignControl'), status: <StatusBadge status={selectedVoucher.status} />, fallback: { logoUrl: null, logoAlt: '', initials: (selectedVoucher.outlets?.name || selectedVoucher.name).slice(0, 2).toUpperCase() } }}><span className="text-xs font-semibold text-gray-500">{t('ui.vouchers.voucherDetails')}</span></VoucherTicket></div><div className="mt-6 space-y-3 text-sm"><div className="grid grid-cols-2 gap-3"><div className="rounded-xl bg-gray-50 p-4"><p className="text-xs text-gray-500">{t('ui.vouchers.usage')}</p><p className="mt-1 font-semibold text-gray-900">{selectedVoucher.uses_count} {selectedVoucher.max_uses ? `/ ${selectedVoucher.max_uses}` : t('ui.vouchers.uses')}</p></div><div className="rounded-xl bg-gray-50 p-4"><p className="text-xs text-gray-500">{t('ui.vouchers.status')}</p><div className="mt-1"><StatusBadge status={selectedVoucher.status} /></div></div></div><p className="text-gray-600">{t('ui.vouchers.outletValue', { outlet: selectedVoucher.outlets?.name || t('ui.vouchers.allOutlets') })}</p></div><div className="mt-7 flex gap-2"><button type="button" onClick={() => void copyCode(selectedVoucher.code)} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-amber-600 px-4 py-2.5 text-sm font-semibold text-white">{copiedCode === selectedVoucher.code ? <Check size={15} /> : <Copy size={15} />} {copiedCode === selectedVoucher.code ? t('ui.vouchers.copied') : t('ui.vouchers.copyCode')}</button><button type="button" onClick={() => requestToggle(selectedVoucher)} className={`rounded-xl border px-4 py-2.5 text-sm font-semibold ${selectedVoucher.is_active ? 'border-red-200 text-red-600 hover:bg-red-50' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{selectedVoucher.is_active ? t('ui.vouchers.deactivate') : t('ui.vouchers.activate')}</button></div></aside></div>}
