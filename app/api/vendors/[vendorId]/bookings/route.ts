...
export async function GET(request: Request, { params }: Props) {
  ...
  if (!vendor || vendor.owner_id !== user.id || vendor.status !== 'approved') return apiFail('FORBIDDEN', 'You cannot view this vendor bookings list', 403);
  ...
  const { data: outlets, error: outletError } = await service.from('outlets').select('id').eq('vendor_id', vendorId);
  ...
  let query = service.from('bookings').select('id,status,created_at,check_in_at,demo_qr_code,customer_id,slot_id,order_item_id,order_items!inner(product_name,quantity,line_total,slot_starts_at,product_id,outlet_id,products(name,cover_url),outlets(name,city,state)),users(full_name,email),booking_slots!inner(starts_at,ends_at,capacity,booked,products(name,cover_url),outlets(name,city,state))', { count: 'exact' }).in('slot_id', slotIds);
  ...
  if (q) {
    const [{ data: matchingUsers }, { data: matchingItems }, { data: bookingIds }] = await Promise.all([
      ...
      service.from('order_items').select('id').eq('vendor_id', vendorId).ilike('product_name', `%${q}%`).limit(100),
      service.from('bookings').select('id').in('slot_id', slotIds).limit(10000),
    ]);
    const userIds = (matchingUsers || []).map((item: any) => item.id);
    const itemIds = (matchingItems || []).map((item: any) => item.id);
    const matchingBookingIds = (bookingIds || []).filter((item: any) => String(item.id).toLowerCase().includes(rawQ.toLowerCase())).map((item: any) => item.id);
    const conditions = [];
  ...
  const items = (data || []).map((booking: any) => ({
    ...
    orderItem: Array.isArray(booking.order_items) ? booking.order_items[0] : booking.order_items,
    slot: Array.isArray(booking.booking_slots) ? booking.booking_slots[0] : booking.booking_slots,
  }));
