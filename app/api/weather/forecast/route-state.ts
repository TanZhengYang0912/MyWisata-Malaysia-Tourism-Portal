...
export function consumeWeatherForecastRateLimit(userId: string, now = Date.now()) {
...
export async function withWeatherForecastProviderSlot<T>(work: () => Promise<T>) {
...
export function __resetWeatherRouteStateForTests() {
  userRequests.clear();
