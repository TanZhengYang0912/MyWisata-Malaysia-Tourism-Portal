...
export async function POST() {
  ...
  const { data: seenAt, error } = await supabase.rpc('mark_my_recommendations_seen');
  if (error || typeof seenAt !== 'string') {
    return apiFail('DB_ERROR', 'Unable to update recommendation unread state', 500);
  }
