...
describe('POST /api/admin/withdrawals/:id/resume', () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.getUser.mockResolvedValue({ data: { user: { id: actor } }, error: null }); mocks.moderateWalletAction.mockResolvedValue({ ok: true, categories: [] }); mocks.enqueueWithdrawalEmail.mockResolvedValue(undefined); mocks.rpc.mockImplementation((name: string) => name === 'has_staff_permission' ? Promise.resolve({ data: true, error: null }) : Promise.resolve({ data: null, error: null })); });
  it('requires a category and a ten-character reason', async () => { const response = await POST(request({ reason: 'short' }), { params: Promise.resolve({ id }) }); expect(response.status).toBe(422); expect(mocks.rpc).not.toHaveBeenCalledWith('resume_wallet_withdrawal', expect.anything()); });
  it('resumes a held withdrawal and sends the fresh cycle to email', async () => {
    mocks.rpc.mockImplementation((name: string) => name === 'has_staff_permission' ? Promise.resolve({ data: true, error: null }) : Promise.resolve({ data: { user_id: actor, amount_rm: 100, approval_cycle: 2 }, error: null }));
