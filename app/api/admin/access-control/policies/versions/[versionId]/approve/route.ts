import { approvePolicyVersion } from "@/lib/entitlements/admin";
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    await approvePolicyVersion(versionId, parsed.data.reason);
    return mutationReceipt(db, user.id, "entitlement.policy_version.approved", versionId, { policyVersionId: versionId });
  } catch (error) {
