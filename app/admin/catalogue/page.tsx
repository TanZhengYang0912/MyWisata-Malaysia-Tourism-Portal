    ...
    <main className="min-h-full px-4 py-6 sm:px-6 sm:py-8 xl:px-8">
      <div className="w-full space-y-6">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><ClipboardCheck size={15} /> {t('catalogue.eyebrow')}</p>
            <h1 className="mt-2 font-[family-name:var(--font-display)] text-3xl font-bold tracking-[-0.04em] text-foreground sm:text-4xl">{t('catalogue.title')}</h1>
            <p className="mt-1 text-sm text-gray-500">{t('catalogue.description')}</p>
          </div>
          <div className="rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3 text-right"><p className="text-sm font-semibold text-amber-700">{t('catalogue.waiting')}</p><p className="mt-4 text-3xl font-bold tracking-[-0.05em] text-amber-950">{items.length}</p></div>
        </header>
