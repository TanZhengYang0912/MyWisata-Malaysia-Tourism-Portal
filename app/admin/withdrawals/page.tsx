...
export default function AdminWithdrawalsPage() {
  ...
  const displayStatus = (value: string | null | undefined) => value
    ? t(ENUM_VALUE_KEYS[value] ?? "withdrawals.status.unknown", { defaultValue: value })
  ...
  return <div className="min-h-full bg-background px-4 py-6 sm:px-6 sm:py-8 xl:px-8">
    ...
    <div className="rounded-2xl overflow-hidden bg-card border border-border shadow-[0_1px_10px_rgba(1,0,102,0.06)]">
      ...
      <div className="px-5 py-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground"><span>{t("withdrawals.pagination.pageOf", { page, totalPages: Math.max(totalPages, 1) })}</span>{totalPages > 1 && <div className="flex gap-2"><Button size="sm" variant="outline" aria-label={tCommon("accessibility.previousPage", { defaultValue: "Previous page" })} disabled={page <= 1 || loading} onClick={() => setPage((value) => value - 1)}><ChevronLeft size={14} /></Button><Button size="sm" variant="outline" aria-label={tCommon("accessibility.nextPage", { defaultValue: "Next page" })} disabled={page >= totalPages || loading} onClick={() => setPage((value) => value + 1)}><ChevronRight size={14} /></Button></div>}</div>
    </div>
