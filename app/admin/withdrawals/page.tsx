...
type Detail = ListItem & {
  ...
  payoutExecution: { locked: boolean; claimedAt: string | null };
};
...
type Action = "approve" | "hold" | "reject" | "resume" | "fraud-override" | "retry-payout";
...
const DECISION_COPY: Record<Action, { labelKey: string; descriptionKey: string; consequenceKey: string; placeholderKey: string }> = {
  ...
  "retry-payout": {
    labelKey: "withdrawals.decisions.retryPayout.label",
    descriptionKey: "withdrawals.decisions.retryPayout.description",
    consequenceKey: "withdrawals.decisions.retryPayout.consequence",
    placeholderKey: "withdrawals.decisions.retryPayout.placeholder",
  },
};
...
function toWalletReasonAction(action: Action): WalletReasonAction {
  if (action === "fraud-override") return "fraud_override";
  if (action === "retry-payout") return "approve";
  return action;
}
...
function getPayoutFailureNextAction(failure: Detail['payoutFailure']) {
  if (failure.retryable) return 'withdrawals.payoutFailure.nextAction.retryable';
  if (failure.category === 'invalid_destination' || failure.category === 'account_disabled') {
    return 'withdrawals.payoutFailure.nextAction.invalidDestination';
  }
  return 'withdrawals.payoutFailure.nextAction.reconcile';
}
...
export default function AdminWithdrawalsPage() {
  ...
  async function openDetail(id: string) {
    ...
    try {
      ...
      const data = body.data as Detail & { reviewSources?: Detail["reviewSources"]; payoutFailure?: Detail["payoutFailure"]; payoutExecution?: Detail["payoutExecution"] };
      setDetail({
        ...data,
        ...
        payoutExecution: data.payoutExecution ?? { locked: false, claimedAt: null },
      });
      ...
      setError(message);
      showFeedback("error", `${message}. ${t("withdrawals.feedback.refreshAndAskSuperAdmin")}`);
    }
  }
  ...
  async function confirmAction() {
    ...
    const payload = nextAction === "approve" || nextAction === "retry-payout"
  ...
  const canRetryPayout = detail?.status === "approved" && detail.payoutFailure.retryable !== false && !detail.payoutExecution.locked;
  ...
  const canChooseDecision = Boolean(canApprove || canHoldReject || canResume || canRetryPayout || isSuperAdmin);
  const availableDecisions: Action[] = [
    ...
    ...(canRetryPayout ? ["retry-payout" as const] : []),
  ...
  async function applyBatch(action: Action) {
    ...
    if (!window.confirm(t("withdrawals.prompts.batchConfirm", {
      action: t(DECISION_COPY[action].labelKey),
  ...
  return <div className="min-h-full bg-background px-4 py-6 sm:px-6 sm:py-8 xl:px-8">
    ...
    <div aria-label={t("withdrawals.accessibility.reviewSummary")} className="grid grid-cols-2 gap-3 xl:grid-cols-5">
      {[{ key: "needsAction", labelKey: "withdrawals.metrics.needsAction", value: total, detail: t("withdrawals.metrics.requestsInQueue") }, { key: "pendingValue", labelKey: "withdrawals.metrics.pendingPayoutValue", value: formatRM(visiblePayoutValue), detail: t("withdrawals.metrics.visiblePageTotal") }, { key: "highRisk", labelKey: "withdrawals.metrics.highRisk", value: visibleHighRisk, detail: t("withdrawals.metrics.visiblePageTotal") }, { key: "dualApproval", labelKey: "withdrawals.metrics.dualApproval", value: visibleDualApproval, detail: t("withdrawals.metrics.waitingForSecondApprover") }, { key: "overdue", labelKey: "withdrawals.metrics.overdue", value: visibleOverdue, detail: oldestRequest ? t("withdrawals.metrics.oldestRequest", { age: formatAge(oldestRequest.createdAt) }) : t("withdrawals.metrics.noOverdueRequests") }].map((metric) => <div key={metric.key} className="rounded-2xl border border-border bg-card p-5 shadow-[0_1px_10px_rgba(1,0,102,0.06)]"><p className="text-sm font-semibold text-muted-foreground">{t(metric.labelKey)}</p><p className="mt-4 text-3xl font-bold tracking-[-0.05em] text-foreground">{metric.value}</p><p className="mt-1 text-xs font-medium text-muted-foreground">{metric.detail}</p></div>)}
    </div>
    ...
    <div className="rounded-2xl overflow-hidden bg-card border border-border shadow-[0_1px_10px_rgba(1,0,102,0.06)]">
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[980px]">
          <div className="flex items-center gap-2 border-b border-border bg-muted/30 px-5 py-3 text-xs"><input type="checkbox" aria-label={t("withdrawals.accessibility.selectAllVisible")} checked={items.length > 0 && items.every((item) => selectedIds.has(item.id))} onChange={(event) => setSelectedIds(event.target.checked ? new Set(items.map((item) => item.id)) : new Set())} /><span className="text-muted-foreground">{t("withdrawals.selection.selectAllOnPage")}</span></div>
      ...
      <div className="px-5 py-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground"><span>{t("withdrawals.pagination.pageOf", { page, totalPages: Math.max(totalPages, 1) })}</span>{totalPages > 1 && <div className="flex gap-2"><Button size="sm" variant="outline" disabled={page <= 1 || loading} onClick={() => setPage((value) => value - 1)}><ChevronLeft size={14} /></Button><Button size="sm" variant="outline" disabled={page >= totalPages || loading} onClick={() => setPage((value) => value + 1)}><ChevronRight size={14} /></Button></div>}</div>
    </div>
    {detail && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-4 sm:p-6" role="dialog" aria-modal="true" aria-label={t("withdrawals.accessibility.detailDialog", { customer: detail.customer.displayName })} onClick={() => setDetail(null)}><aside className="max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto rounded-2xl border border-border bg-card p-5 shadow-2xl sm:max-h-[calc(100vh-3rem)] sm:p-6" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="rounded-xl border border-border p-4 mt-4 text-sm"><p className="font-semibold mb-2">{t("withdrawals.balances.title")}</p><p>{t("withdrawals.balances.topUp")}: RM {(detail.wallet.topupSen / 100).toFixed(2)}</p><p>{t("withdrawals.balances.earnings")}: RM {(detail.wallet.earningsSen / 100).toFixed(2)}</p><p>{t("withdrawals.balances.pendingRewards")}: RM {(detail.wallet.pendingEarningsSen / 100).toFixed(2)}</p><p>{t("withdrawals.balances.reserved")}: RM {(detail.wallet.reservedSen / 100).toFixed(2)}</p><p>{t("withdrawals.balances.withdrawn")}: RM {(detail.wallet.withdrawnSen / 100).toFixed(2)}</p><p className="mt-2 text-muted-foreground">{t("withdrawals.readiness.payoutDestination")}: {detail.destinationLabel}</p></div>
      ...
      {(detail.payoutFailure.code || detail.payoutFailure.message || detail.payoutFailure.category) && <section role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 mt-4 text-sm"><p className="font-semibold mb-2">{t("withdrawals.payoutFailure.title")}</p><p>{t("withdrawals.payoutFailure.provider")}: {detail.payoutFailure.provider ?? t("withdrawals.detail.unknown")}</p><p>{t("withdrawals.payoutFailure.category")}: {displayStatus(detail.payoutFailure.category)}</p><p>{t("withdrawals.payoutFailure.code")}: {detail.payoutFailure.code ?? t("withdrawals.detail.notSupplied")}</p><p>{t("withdrawals.payoutFailure.reason")}: {detail.payoutFailure.message ?? t("withdrawals.payoutFailure.noProviderMessage")}</p><p>{t("withdrawals.payoutFailure.retryable")}: {detail.payoutFailure.retryable ? t("withdrawals.common.yes") : t("withdrawals.common.no")}</p><p className="mt-2 font-semibold">{t("withdrawals.payoutFailure.recommendedNextAction")}</p><p>{t(getPayoutFailureNextAction(detail.payoutFailure))}</p></section>}
      {detail.payoutExecution.locked && <section role="alert" className="rounded-xl border border-amber-300 bg-amber-50 p-4 mt-4 text-sm text-amber-950"><p className="font-semibold">{t("withdrawals.payoutExecution.locked")}</p><p className="mt-1">{t("withdrawals.payoutExecution.lockedDescription")}</p>{detail.payoutExecution.claimedAt && <p className="mt-2 text-xs">{t("withdrawals.payoutExecution.lockedSince", { date: new Date(detail.payoutExecution.claimedAt).toLocaleString(locale) })}</p>}</section>}
