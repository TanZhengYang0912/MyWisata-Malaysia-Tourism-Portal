    ...
    <section className="rounded-2xl border border-border bg-card p-4 sm:p-5">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:w-auto">
          <select aria-label={t("ui.users.filters.roleLabel")} value={filters.role ?? ""} onChange={(event) => changeFilter("role", event.target.value ? event.target.value as UserManagementFilters["role"] : null)} className="h-10 rounded-xl border border-border bg-background px-2 text-xs"><option value="">{t("ui.users.filters.allRoles")}</option><option value={t("chatReports.roles.customer")}>{t("ui.users.roles.customer")}</option><option value="vendor_owner">{t("ui.users.roles.vendorOwner")}</option><option value="outlet_manager">{t("ui.users.roles.outletManager")}</option></select>
