...
export default function AdminUsersPage() {
  ...
  return <main className="min-h-full bg-background px-4 py-6 sm:px-6 sm:py-8 xl:px-8">
    <header>
      <div className="flex items-center gap-2 text-primary"><UsersRound size={18} /><p className="text-xs font-semibold uppercase tracking-[0.18em]">{t("ui.users.eyebrow")}</p></div>
      <h1 className="mt-2 font-[family-name:var(--font-display)] text-3xl font-bold tracking-[-0.04em] text-foreground sm:text-4xl">{t("ui.users.title")}</h1>
      <p className="mt-1 max-w-2xl text-sm text-muted-foreground">{t("ui.users.description")}</p>
    </header>

    <section className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {[[t("ui.users.stats.total"), result.total, t("ui.users.stats.totalNote")], [t("ui.users.stats.pendingKyc"), pageStats.pendingKyc, t("ui.users.stats.pageNote")], [t("ui.users.stats.incomplete"), pageStats.incomplete, t("ui.users.stats.pageNote")], [t("ui.users.stats.attention"), pageStats.restricted, t("ui.users.stats.attentionNote")]].map(([label, value, note]) => <div key={label} className="rounded-2xl border border-border bg-card p-5"><p className="text-sm font-semibold text-muted-foreground">{label}</p><p className="mt-4 text-3xl font-bold tracking-[-0.05em] text-foreground">{value}</p><p className="mt-1 text-xs font-medium text-muted-foreground">{note}</p></div>)}
    </section>

    <section className="rounded-2xl border border-border bg-card p-4 sm:p-5">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
        <form onSubmit={submitSearch} className="flex min-w-0 flex-1 gap-2">
          <div className="relative min-w-0 flex-1"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder={t("ui.users.searchPlaceholder")} className="h-10 w-full rounded-xl border border-border bg-background pl-9 pr-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/15" /></div>
          <Button type="submit" size="sm">{t("ui.actions.search")}</Button>
        </form>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:w-auto">
          <select aria-label={t("ui.users.filters.roleLabel")} value={filters.role ?? ""} onChange={(event) => changeFilter("role", event.target.value ? event.target.value as UserManagementFilters["role"] : null)} className="h-10 rounded-xl border border-border bg-background px-2 text-xs"><option value="">{t("ui.users.filters.allRoles")}</option><option value="customer">{t("ui.users.roles.customer")}</option><option value="vendor_owner">{t("ui.users.roles.vendorOwner")}</option><option value="outlet_manager">{t("ui.users.roles.outletManager")}</option></select>
          <select aria-label={t("ui.users.filters.statusLabel")} value={filters.status ?? ""} onChange={(event) => changeFilter("status", event.target.value ? event.target.value as UserManagementFilters["status"] : null)} className="h-10 rounded-xl border border-border bg-background px-2 text-xs"><option value="">{t("ui.users.filters.allStatuses")}</option><option value="active">{t("ui.users.status.active")}</option><option value="suspended">{t("ui.users.status.suspended")}</option><option value="deleted">{t("ui.users.status.deleted")}</option></select>
          <select aria-label={t("ui.users.filters.kycLabel")} value={filters.kycStatus ?? ""} onChange={(event) => changeFilter("kycStatus", event.target.value ? event.target.value as UserManagementFilters["kycStatus"] : null)} className="h-10 rounded-xl border border-border bg-background px-2 text-xs"><option value="">{t("ui.users.filters.allKyc")}</option><option value="unverified">{t("ui.users.status.unverified")}</option><option value="pending">{t("ui.users.status.pending")}</option><option value="approved">{t("ui.users.status.approved")}</option><option value="rejected">{t("ui.users.status.rejected")}</option></select>
          <select aria-label={t("ui.users.filters.bioLabel")} value={filters.bioLocked === null ? "" : String(filters.bioLocked)} onChange={(event) => changeFilter("bioLocked", event.target.value === "" ? null : event.target.value === "true")} className="h-10 rounded-xl border border-border bg-background px-2 text-xs"><option value="">{t("ui.users.filters.allBioStates")}</option><option value="true">{t("ui.users.bio.locked")}</option><option value="false">{t("ui.users.bio.clear")}</option></select>
        </div>
      </div>
    ...
    <section className="mt-5 overflow-hidden rounded-2xl border border-border bg-card">
      ...
      {/* eslint-disable-next-line @next/next/no-img-element, @typescript-eslint/no-unused-expressions */}
