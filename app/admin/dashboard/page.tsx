...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="min-h-full p-6 sm:p-8 xl:p-10">
      <div className="mx-auto max-w-[1500px]">
        <header className="mb-7 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            ...
            <h1 className="font-[family-name:var(--font-display)] text-3xl font-bold tracking-tight text-foreground sm:text-4xl">Admin Overview</h1>
        ...
        {loadError && <div role="alert" className="mb-5 flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800"><AlertCircle size={17} /> {loadError}</div>}

        {!data ? <p className="text-sm text-muted-foreground">Loading governance queues…</p> : <>
          <section aria-label="Approval summary" className="grid grid-cols-2 gap-4 xl:grid-cols-5">
            {[
              ...
              <div key={metric.label} className="relative overflow-hidden rounded-2xl border border-border bg-card p-5 shadow-[0_1px_10px_rgba(1,0,102,0.07)]">
                ...
                <p className="font-[family-name:var(--font-mono)] text-2xl font-bold tracking-tight text-foreground">{metric.value}</p>
                <p className="mt-1 text-xs font-semibold text-foreground">{metric.label}</p>
                <p className="mt-1 text-[11px] leading-4 text-muted-foreground">{metric.supporting}</p>
              </div>
            ))}
          </section>

          <section className="mt-6 grid gap-5 xl:grid-cols-[1.45fr_0.9fr_0.8fr]">
