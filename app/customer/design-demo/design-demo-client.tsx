...
function DemoVendorCard({ vendor, index }: { vendor: DemoVendor; index: number }) {
  ...
  const businessType = vendor.businessType
    ? tCustomer(`ui.vendor.businessTypes.${vendor.businessType}`, { defaultValue: formatBusinessType(vendor.businessType) })
      ...
      <Link href={`/customer/vendor/${vendor.id}`} className="block focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20">
        <div className="relative aspect-[1.5] overflow-hidden bg-secondary">
          {visual.coverUrl ? <>
            ...
            <img src={visual.coverUrl} alt={tCustomer("ui.home.vendorCover", { vendor: vendor.name, defaultValue: "{{vendor}} cover" })} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" />
...
export function DesignDemoClient({ activities, recommended, vendors, initialState }: { activities: ComputedActivity[]; recommended: ComputedActivity[]; vendors: DemoVendor[]; initialState?: string }) {
  const { t: tCustomer } = useTranslation("customer");
  const tx = (key: string, defaultValue: string, options?: Record<string, unknown>) => tCustomer(key, { ...options, defaultValue });
  ...
  const translatedPlaceholders = useMemo(() => PLACEHOLDER_TEXTS.map((fallback, index) => (
    tCustomer(`ui.designDemo.placeholder${index}`, { defaultValue: fallback })
  )), [tCustomer]);
  ...
  const localizedGuides = useMemo(() => CITY_GUIDES.map((guide, index) => ({
    ...guide,
    label: tCustomer(`ui.designDemo.guide${index}.label`, { defaultValue: guide.label }),
    eyebrow: tCustomer(`ui.designDemo.guide${index}.eyebrow`, { defaultValue: guide.eyebrow }),
    description: tCustomer(`ui.designDemo.guide${index}.description`, { defaultValue: guide.description }),
    detail: tCustomer(`ui.designDemo.guide${index}.detail`, { defaultValue: guide.detail }),
  })), [tCustomer]);
      ...
      <section className="relative isolate min-h-[calc(100svh-64px)] overflow-hidden bg-primary text-white">
        ...
        <div className="mx-auto max-w-7xl px-4 pb-10 pt-8 sm:px-6 sm:pb-12 sm:pt-10 lg:px-8 lg:pb-8">
          <div className="mb-8 flex flex-wrap items-center justify-between gap-4 lg:mb-7">
            <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-[0.2em] text-white/60"><Compass size={16} aria-hidden="true" className="text-[#ffcc00]" /> MyWisata / Atlas</div>
            ...
            <div className="atlas-enter atlas-delay-3 relative mx-auto min-h-[540px] w-full max-w-[460px] md:min-h-[500px] md:max-w-[390px] lg:min-h-[510px] lg:max-w-[500px]">
              ...
              <div key={activeDestination.state} className="atlas-active-card absolute bottom-0 right-0 z-20 w-full overflow-hidden rounded-[30px] border border-white/20 bg-black/20 shadow-[0_28px_70px_rgba(0,0,0,0.35)] lg:w-[82%]">
                <div className="relative aspect-[0.78]">
                  ...
                  <div className="atlas-mobile-spotlight absolute right-5 top-5 z-30 rounded-2xl border border-white/20 bg-[#00004d]/90 px-3 py-2.5 text-right shadow-lg backdrop-blur lg:hidden"><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-white/45">{tCustomer("ui.home.spotlight")}</p><p className="mt-1 text-xs font-bold text-white">{activeDestination.state}</p><p className="mt-1 hidden text-[10px] text-[#ffcc00] sm:block">{activeGuide} plan</p></div>
                  <div className="atlas-desktop-spotlight absolute right-5 top-5 z-30 hidden rounded-2xl border border-white/20 bg-[#00004d]/90 px-4 py-3 text-right shadow-lg backdrop-blur lg:block"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-white/45">{tCustomer("ui.home.spotlight")}</p><p className="mt-1 text-sm font-bold text-white">{activeDestination.state}</p><p className="mt-1 text-[11px] text-[#ffcc00]">{activeGuide} plan</p></div>
