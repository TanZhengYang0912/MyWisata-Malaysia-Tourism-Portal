...
export default function WithdrawalReceiptPage() {
  ...
  return <><CustomerPageTitle eyebrow={tCustomer("accountGroups.account")} title={tCustomer("strictMigration.walletReceipt.title")} description={receipt?.reference ?? tCustomer("strictMigration.walletReceipt.eyebrow")} icon={<ReceiptText size={14} />} /><CustomerPageShell className="max-w-2xl pt-0 sm:pt-0 print:px-0">
    ...
    {loading ? <div className="rounded-2xl border border-border bg-card p-10 text-center text-muted-foreground">{tCustomer("ui.states.loadingReceipt")}</div> : error ? <div role="alert" className="rounded-2xl border border-destructive/30 bg-card p-10 text-center text-destructive"><p>{error}</p><p className="mt-2 text-sm">{tCustomer("ui.states.loadingError")}</p></div> : receipt && <section className="rounded-2xl border border-border bg-card p-6 sm:p-8">
      ...
      <div className="grid grid-cols-2 gap-4 py-6 text-sm"><div><p className="text-xs text-muted-foreground">{tCustomer("ui.checkout.total")}</p><p className="mt-1 font-mono text-xl font-bold">{formatMYR(receipt.amountRm)}</p></div><div><p className="text-xs text-muted-foreground">{tCustomer("ui.labels.status")}</p><div className="mt-2"><StatusBadge status={receipt.status} /></div></div><div><p className="text-xs text-muted-foreground">{tCustomer("ui.labels.date")}</p><p className="mt-1">{new Date(receipt.createdAt).toLocaleString(i18n.resolvedLanguage)}</p></div><div><p className="text-xs text-muted-foreground">{tCustomer("ui.labels.history")}</p><p className="mt-1">{new Date(receipt.updatedAt).toLocaleString(i18n.resolvedLanguage)}</p></div></div>
      ...
      {movement && <section className="mt-4 rounded-xl border border-border p-4 text-sm">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <div className="rounded-lg bg-muted px-3 py-2"><p className="text-xs text-muted-foreground">{tCustomer("strictMigration.walletReceipt.moneyMovement.reserved")}</p><p className="font-mono font-semibold">{formatMYRFromSen(movement.amountSen)}</p>{!movement.to && <p className="mt-1 text-xs text-muted-foreground">{movementDestination}</p>}</div>
          {movement.to && <><ArrowRight size={18} className="text-muted-foreground" /><div className="rounded-lg bg-primary/10 px-3 py-2"><p className="text-xs text-muted-foreground">{movementDestination}</p><p className="font-mono font-semibold">{formatMYRFromSen(movement.amountSen)}</p></div></>}
        </div>
      </section>}
