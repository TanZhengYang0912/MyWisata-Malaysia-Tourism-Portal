...
export default function OrderDetailPage() {
  ...
  return (
    <div className="order-receipt-page mx-auto max-w-3xl px-4 py-8 sm:px-6 print:max-w-none print:px-0 print:py-0">
      <div className="mb-8">
        <Link href="/customer/activity?tab=orders" className="text-sm font-semibold text-primary hover:underline mb-4 inline-block print:hidden">← Back to Orders</Link>
        {order.status === "PAID" || order.status === "COMPLETED" ? (
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-foreground tracking-tight">Booking Confirmed!</h1>
              <p className="text-sm text-muted-foreground mt-1">Your adventure is locked in.</p>
            </div>
          </div>
        ) : (
      ...
      {bookings.length > 0 && (
        <div className="space-y-4 mb-8">
          <p className="text-lg font-bold text-foreground tracking-tight">Your Bookings</p>
          {bookings.map((b) => (
