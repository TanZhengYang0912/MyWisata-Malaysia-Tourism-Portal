    ...
    <div className="min-h-full bg-background">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="mb-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><CalendarDays size={14} /> {viewCopy.eyebrow}</p>
            <h1 className="font-[family-name:var(--font-display)] text-3xl font-bold tracking-tight text-foreground sm:text-4xl">{viewCopy.title}</h1>
            <p className="mt-2 max-w-xl text-sm text-muted-foreground">{viewCopy.description}</p>
          </div>
        </header>
        <section aria-label="Booking calendar" className="overflow-hidden rounded-3xl border border-border bg-card shadow-sm">
          <div className="border-b border-border">
            <div className="flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
              ...
              <div className="relative text-center">
                ...
                {monthPickerOpen && <div id="calendar-month-picker" role="dialog" aria-label="Choose calendar month and year" className="absolute left-1/2 top-full z-30 mt-2 w-[min(22rem,calc(100vw-2rem))] -translate-x-1/2 rounded-2xl border border-border bg-card p-4 text-left shadow-xl">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <label className="flex items-center gap-2 text-xs font-semibold text-slate-500">
                      ...
                      <select value={monthStart.getFullYear()} onChange={(event) => selectCalendarYear(Number(event.target.value))} aria-label="Choose year" className="h-9 rounded-lg border border-border bg-card px-2 text-sm font-bold text-primary outline-none focus:border-primary focus:ring-2 focus:ring-primary/20">
            ...
            <div aria-label="Calendar actions" className="flex items-center justify-between gap-3 border-t border-border px-4 py-2.5 sm:px-6">
              <Link href={activityHref("orders")} className="inline-flex h-9 items-center gap-1.5 rounded-full border border-primary/20 bg-card px-3 text-xs font-bold text-primary hover:bg-secondary"><ReceiptText size={14} /> Orders & receipts</Link>
              <div className="flex items-center justify-end gap-2">
                <Button variant="outline" onClick={goToToday} className="h-9 rounded-full border-primary/20 bg-card px-3 text-xs text-primary hover:bg-secondary">Today</Button>
            ...
            {filtersOpen && <div id="calendar-filters" className="border-t border-border px-4 py-3 sm:px-6"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-6"><label className="relative min-w-0 lg:col-span-2"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(event) => { setSelectedDayKey(null); setQuery(event.target.value); }} placeholder="Search bookings" aria-label="Search activity, outlet or order ID" className="h-10 w-full rounded-xl border border-border bg-secondary/50 pl-9 pr-3 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-[rgba(1,0,102,0.12)]" /></label><select value={scope} onChange={(event) => { setSelectedDayKey(null); setScope(event.target.value as BookingScope); }} aria-label="Booking scope" className="h-10 rounded-xl border border-border bg-card px-3 text-sm text-foreground outline-none focus:border-primary"><option value="upcoming">Upcoming</option><option value="past">Past</option><option value="all">All bookings</option></select><select value={outletId} onChange={(event) => { setSelectedDayKey(null); setOutletId(event.target.value); }} aria-label="Booking outlet" className="h-10 rounded-xl border border-border bg-card px-3 text-sm text-foreground outline-none focus:border-primary"><option value="all">All outlets</option>{outlets.map((outlet) => <option key={outlet.id} value={outlet.id}>{outlet.name}</option>)}</select><select value={activityId} onChange={(event) => { setSelectedDayKey(null); setActivityId(event.target.value); }} aria-label="Booking activity" className="h-10 rounded-xl border border-border bg-card px-3 text-sm text-foreground outline-none focus:border-primary"><option value="all">All activities</option>{activityOptions.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select><input type="date" value={from} onChange={(event) => { setSelectedDayKey(null); setFrom(event.target.value); }} aria-label="Bookings from date" className="h-10 rounded-xl border border-border bg-card px-3 text-sm text-foreground outline-none focus:border-primary" /><input type="date" value={to} onChange={(event) => { setSelectedDayKey(null); setTo(event.target.value); }} aria-label="Bookings to date" className="h-10 rounded-xl border border-border bg-card px-3 text-sm text-foreground outline-none focus:border-primary" /></div>{hasFilters && <button type="button" onClick={clearFilters} className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-primary hover:underline"><X size={13} /> Clear filters</button>}</div>}
          </div>
          {error && <div className="m-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 sm:m-5">{error}</div>}
            <div className="hidden md:block"><div className="grid grid-cols-7 border-b border-border bg-secondary/50">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => <div key={day} className="px-3 py-2.5 text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">{day}</div>)}</div><div className="grid grid-cols-7">{calendarDays.map((day) => { const key = `${day.getFullYear()}-${String(day.getMonth() + 1).padStart(2, "0")}-${String(day.getDate()).padStart(2, "0")}`; const dayGroups = bookingsByDay[key] || []; const inMonth = day.getMonth() === monthStart.getMonth() && day.getFullYear() === monthStart.getFullYear(); const hiddenCount = getHiddenItineraryGroupCount(dayGroups); return <div key={key} className={`min-h-[104px] border-b border-r border-border p-2 ${inMonth ? "bg-card" : "bg-secondary/20"}`}><p className={`mb-1.5 text-xs font-bold ${inMonth ? "text-foreground" : "text-muted-foreground/50"}`}>{day.getDate()}</p><div className="space-y-1">{dayGroups.slice(0, 3).map((group) => <button key={group.key} type="button" onClick={() => setSelectedDayKey(key)} className="block w-full rounded-lg border-l-2 border-primary bg-secondary px-2 py-1 text-left transition hover:bg-secondary/70 focus:outline-none focus:ring-2 focus:ring-primary/30" aria-label={`Show ${group.activityName} itinerary details`}><p className="truncate text-[11px] font-bold text-primary">{group.activityName}</p><p className="mt-0.5 text-[10px] text-primary">{formatBookingTime(group.slotStartsAt)} · {group.totalQty} pax</p></button>)}{hiddenCount > 0 && <button type="button" onClick={() => setSelectedDayKey(key)} aria-label={`Show ${hiddenCount} more itinerary items for ${formatCalendarDate(day)}`} className="px-2 text-left text-[10px] font-bold text-primary underline-offset-2 hover:underline focus:outline-none focus:ring-2 focus:ring-primary/30">+{hiddenCount} more</button>}</div></div>; })}</div></div><div className="space-y-3 p-4 md:hidden">{itineraryGroups.length === 0 ? <EmptyState icon={<CalendarDays size={36} />} title="No bookings found" description={hasFilters ? "Try changing your filters." : "Your booked activities will appear here after checkout."} /> : itineraryGroups.map((group) => <button key={group.key} type="button" onClick={() => group.slotStartsAt && setSelectedDayKey(calendarDateKey(new Date(group.slotStartsAt)))} className="block w-full rounded-2xl border border-border bg-secondary/50 p-4 text-left transition hover:border-primary/30 focus:outline-none focus:ring-2 focus:ring-primary/30"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-foreground">{group.activityName}</p><p className="mt-1 text-xs font-semibold text-primary">{formatBookingDate(group.slotStartsAt)} · {formatBookingTime(group.slotStartsAt)}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${statusClass(group.status)}`}>{statusLabel(group.status)}</span></div><p className="mt-3 flex items-center gap-1 text-xs text-muted-foreground"><MapPin size={13} /> {outletMap.get(group.outletId)?.name || "MyWisata outlet"} · {group.totalQty} pax · {group.bookings.length} booking{group.bookings.length === 1 ? "" : "s"}</p><p className="mt-2 flex items-center gap-1 text-[11px] text-muted-foreground"><Clock3 size={12} /> View itinerary details</p></button>)}</div>
        </section>
