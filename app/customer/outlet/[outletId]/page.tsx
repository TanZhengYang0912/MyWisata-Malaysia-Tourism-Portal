...
export default async function OutletShopPage({ params }: { params: Promise<{ outletId: string }> }) {
  ...
  return <main className="min-h-screen" style={{ background: '#f8fafc', color: '#0f172a', fontFamily: page?.font_family || 'Inter, sans-serif' }}><section className="relative overflow-hidden" style={{ backgroundColor: page?.brand_colour || '#010066' }}>{page?.hero_url && <img src={page.hero_url} alt="" className="absolute inset-0 h-full w-full object-cover opacity-35" />}<div className="relative mx-auto max-w-5xl px-6 py-24 text-white"><p className="text-xs font-semibold uppercase tracking-[0.2em] opacity-80">Verified MyWisata outlet</p><div className="mt-3 flex flex-wrap items-center gap-3"><h1 className="max-w-3xl text-4xl font-black tracking-tight sm:text-6xl">{outlet.name}</h1><ShareButton shareType="vendor" contentId={outlet.id} title={outlet.name} /><OutletChatButton outletId={outlet.id} /></div><p className="mt-4 max-w-2xl text-sm opacity-90">{outlet.address || [outlet.city, outlet.state].filter(Boolean).join(', ')}</p></div></section><div className="mx-auto max-w-5xl px-6 py-10"><div className="grid gap-6 md:grid-cols-2">{blocks.map((block) => <section key={block.id} className="rounded-2xl border border-primary/10 bg-white p-6 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">{block.type.replace('_', ' ')}</p><h2 className="mt-2 text-xl font-bold">{block.title || 'Discover this outlet'}</h2>{block.body && <p className="mt-2 text-sm leading-6 text-slate-600">{block.body}</p>}{(block.imageUrl || block.image) && <img src={block.imageUrl || block.image} alt={block.title || ''} className="mt-4 h-40 w-full rounded-xl object-cover" />}{block.type === 'hours' && <p className="mt-4 rounded-xl bg-amber-50 px-4 py-3 text-sm text-slate-700">{formatHours(outlet.operating_hours)}</p>}{block.type === 'contact' && <div className="mt-4 rounded-xl bg-secondary px-4 py-3 text-sm text-slate-700"><p>{outlet.address || [outlet.city, outlet.state].filter(Boolean).join(', ') || 'Malaysia'}</p><a className="mt-2 inline-block font-semibold text-primary hover:underline" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([outlet.name, outlet.address, outlet.city, outlet.state].filter(Boolean).join(', '))}`} target="_blank" rel="noreferrer">Open directions</a></div>}{block.type === 'cta' && <a className="mt-4 inline-flex rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary/90" href="#featured-products">{block.cta || 'Explore this outlet'}</a>}{block.type === 'product_grid' && <div id="featured-products" className="mt-4 grid gap-3 sm:grid-cols-2">{featuredProducts.map((product) => <a key={product.id} href={`/customer/activity/${product.id}`} className="overflow-hidden rounded-xl border border-primary/10 bg-secondary/40 transition hover:-translate-y-0.5"><div className="h-28 bg-secondary">{product.cover_url && <img src={product.cover_url} alt={product.name} className="h-full w-full object-cover" />}</div><div className="p-3"><p className="truncate text-sm font-bold">{product.name}</p><p className="mt-1 text-xs font-semibold text-primary">RM {Number(product.base_price).toFixed(2)}</p></div></a>)}</div>}</section>)}</div>{gallery.length > 0 && <section className="mt-8"><h2 className="text-xl font-bold">Gallery</h2><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{gallery.map((item) => <img key={item.url} src={item.url} alt={item.alt || `${outlet.name} gallery`} className="aspect-[4/3] w-full rounded-xl object-cover" />)}</div></section>}<section className="mt-8 rounded-2xl border border-amber-100 bg-amber-50 p-5"><h2 className="font-bold">Opening hours</h2><p className="mt-1 text-sm text-slate-700">{formatHours(outlet.operating_hours)}</p></section></div></main>;
}
