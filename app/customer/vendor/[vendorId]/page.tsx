  ...
  return <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
    ...
    <div className="mt-5 flex flex-col gap-4 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-6 text-sm">
        <span className="font-semibold text-slate-500">{location.listingCount} published experience{location.listingCount === 1 ? '' : 's'}</span>
       ...
       <section id="locations" aria-labelledby="locations-heading">
         <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
           <div>
             ...
             <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{hasMultipleLocations ? 'Each outlet has its own availability and shop page. Select one to see the exact details before booking.' : 'View the address, availability and experiences available at this outlet.'}</p>
           </div>
           {hasMultipleLocations && <span className="text-sm font-semibold text-slate-500">{locations.length} active outlets</span>}
         </div>
