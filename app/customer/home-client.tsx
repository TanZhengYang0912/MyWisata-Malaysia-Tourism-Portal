...
const STATE_REGIONS = [
  { labelKey: "strictMigration.home.regions.north", states: ["Kedah", "Perlis", "Penang", "Perak"] },
  { labelKey: "strictMigration.home.regions.central", states: ["Selangor", "Kuala Lumpur", "Putrajaya"] },
  { labelKey: "strictMigration.home.regions.south", states: ["Johor", "Melaka", "Negeri Sembilan"] },
  { labelKey: "strictMigration.home.regions.eastCoast", states: ["Kelantan", "Pahang", "Terengganu"] },
  { labelKey: "strictMigration.home.regions.borneo", states: ["Sabah", "Sarawak", "Labuan"] },
] as const;
  ...
  return (
    <div>
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
          <div className="flex items-center justify-between gap-4 py-3">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="hidden text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground sm:inline">{tCustomer("strictMigration.home.exploreByState")}</span>
            </div>
            ...
            <span className="hidden text-xs text-muted-foreground md:inline">{tCustomer("strictMigration.home.destinationCount", { count: MALAYSIA_DESTINATIONS.length })}</span>
          </div>
          ...
          {stateMenuOpen && (
            <div className="absolute inset-x-4 top-full z-50 rounded-3xl border border-border bg-card p-4 shadow-2xl sm:inset-x-6 sm:p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-bold text-foreground">{tCustomer("strictMigration.home.chooseRoute")}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{tCustomer("strictMigration.home.routeHint")}</p>
                </div>
                <button type="button" onClick={() => setStateMenuOpen(false)} className="rounded-full p-2 text-muted-foreground transition hover:bg-muted hover:text-foreground" aria-label={tCustomer("strictMigration.home.closeStateMenu")}><X size={16} /></button>
              </div>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <button type="button" onClick={() => { exploreState("All Malaysia"); setStateMenuOpen(false); }} className={`flex items-center justify-between rounded-2xl border px-4 py-3 text-left text-sm font-bold transition ${state === "All Malaysia" ? "border-primary bg-primary text-white" : "border-border bg-background text-foreground hover:border-primary/40"}`}>
                  <span>{tCustomer("ui.search.allMalaysia")}</span>
                  {state === "All Malaysia" && <Check size={16} />}
                </button>
                {STATE_REGIONS.map((region) => (
                  <div key={region.labelKey}>
                    <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">{tCustomer(region.labelKey)}</p>
      ...
      <section id="all-experiences" className="max-w-7xl mx-auto scroll-mt-32 px-4 sm:px-6 py-10">
        <div className="mb-6">
          <h2 className="text-xl sm:text-2xl font-bold text-foreground font-[family-name:var(--font-display)]">{tCustomer("strictMigration.home.browseByCategory")}</h2>
          <p className="text-sm mt-1 text-muted-foreground">{tCustomer("strictMigration.home.categoryPrompt")}</p>
        </div>
          ...
          <div className="flex items-end justify-between mb-6 gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-travel-blue text-xs font-bold uppercase tracking-wider">{personalised ? tCustomer("strictMigration.home.forYou") : tCustomer("strictMigration.home.topPicks")}</span>
              </div>
              <h2 className="text-xl sm:text-2xl font-bold text-foreground font-[family-name:var(--font-display)]">{personalised ? tCustomer("strictMigration.home.recommendedForYou") : tCustomer("strictMigration.home.popularRightNow")}</h2>
            </div>
            <Link href={personalised ? "/customer/preferences" : "/customer/partners"} className="flex items-center gap-1 text-sm font-semibold text-primary shrink-0">
              {personalised ? tCustomer("strictMigration.home.tunePreferences") : tCustomer("strictMigration.home.viewAll")} <ChevronRight size={14} />
            </Link>
          </div>
          {picked.length === 0 ? (
            <EmptyState title={tCustomer("strictMigration.home.noExperiencesMatch")} description={tCustomer("strictMigration.home.tryDifferentStateCategory")} />
          ) : (
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <div className="flex items-end justify-between mb-6 gap-4">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-foreground font-[family-name:var(--font-display)]">
              {state === "All Malaysia" ? tCustomer("strictMigration.home.allExperiences") : tCustomer("strictMigration.home.experiencesIn", { state })}
            </h2>
            <p className="text-sm mt-0.5 text-muted-foreground">{tCustomer("strictMigration.home.resultsCount", { count: activities?.length ?? 0 })}</p>
          </div>
          <Link href="/customer/partners" className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold border border-border text-muted-foreground shrink-0">
            <Search size={12} /> {tCustomer("strictMigration.home.searchFilter")}
          </Link>
        </div>
        {activities === null ? (
          <div className="text-sm text-muted-foreground">{tCustomer("ui.states.loading")}</div>
        ) : activities.length === 0 ? (
          <EmptyState title={tCustomer("strictMigration.home.noExperiencesFound")} description={tCustomer("strictMigration.home.clearFiltersHint")} />
        ) : (
          <div className="grid items-stretch grid-cols-2 gap-4 sm:gap-5 md:grid-cols-4">
        ...
        {activities !== null && activities.length > 0 && totalPages > 1 && (
          <nav aria-label={tCustomer("strictMigration.home.experiencePages")} className="mt-8 flex flex-col gap-4 border-t border-border pt-5 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs text-muted-foreground">
              {tCustomer("strictMigration.home.showingExperiences", { first: pageStart + 1, last: Math.min(pageStart + EXPERIENCES_PER_PAGE, activities.length), total: activities.length })}
            </p>
            <div className="flex items-center gap-1.5">
              <button
                ...
                aria-label={tCustomer("ui.orders.previousPage")}
              ...
              <button
                ...
                aria-label={tCustomer("ui.orders.nextPage")}
                className="flex h-9 w-9 items-center justify-center rounded-xl border border-border text-muted-foreground transition hover:border-primary/30 hover:text-primary disabled:cursor-not-allowed disabled:opacity-35"
              >
