    ...
    <main className="min-h-full bg-background">
      <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-10">
        ...
        <header className="mt-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em] text-primary"><CalendarDays size={14} /> My booking</p>
            ...
            <p className="mt-2 text-sm text-slate-500">Only this booking is shown here. Your other bookings remain in the calendar.</p>
          </div>
        ...
        <section className="mt-7 overflow-hidden rounded-3xl border border-border bg-white shadow-[0_12px_32px_rgba(1,0,102,0.06)]" aria-labelledby="booking-summary-heading">
          ...
          <div className="grid gap-3 p-5 sm:grid-cols-2 sm:p-7">
            <div className="rounded-2xl border border-border bg-slate-50/70 p-4">
              <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-slate-400"><CalendarDays size={14} className="text-primary" /> Date</p>
              <p className="mt-3 text-sm font-bold text-foreground">{formatBookingDate(booking.slotStartsAt)}</p>
            </div>
            <div className="rounded-2xl border border-border bg-slate-50/70 p-4">
              <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-slate-400"><Clock3 size={14} className="text-primary" /> Time</p>
              <p className="mt-3 text-sm font-bold text-foreground">{formatBookingTime(booking.slotStartsAt)}</p>
            </div>
            <div className="rounded-2xl border border-border bg-slate-50/70 p-4">
              <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-slate-400"><MapPin size={14} className="text-primary" /> Location</p>
              <p className="mt-3 text-sm font-bold text-foreground">{outletName}</p>
            </div>
            <div className="rounded-2xl border border-border bg-slate-50/70 p-4">
              <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-slate-400"><Users size={14} className="text-primary" /> Guests</p>
        ...
        <section className="mt-4 flex flex-col gap-5 rounded-3xl border border-border bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:p-7" aria-labelledby="entry-heading">
          ...
          <div className="min-w-0">
            <h2 id="entry-heading" className="text-base font-bold text-foreground">Your entry pass</h2>
            <p className="mt-1 text-sm text-slate-500">Scan this booking pass at the outlet when you arrive.</p>
            <p className="mt-2 truncate text-xs font-semibold text-primary">Booking reference: {booking.id}</p>
          </div>
        </section>

        <div className="mt-6 grid gap-3 sm:grid-cols-2 print:hidden">
          {booking.status === "confirmed" && <Button type="button" variant="outline" className="w-full gap-2" disabled={requestingRefund} onClick={() => void requestRefund()}>{requestingRefund ? "Submitting…" : "Request refund"}</Button>}
          <Button type="button" variant="outline" className="w-full gap-2" onClick={() => window.print()}><Printer size={16} /> Print receipt</Button>
          {showFullReceipt && <Link href={`/customer/orders/${booking.orderId}`} className="w-full"><Button type="button" variant="outline" className="w-full gap-2"><ReceiptText size={16} /> View full receipt</Button></Link>}
        </div>
