...
describe("shareable discovery query", () => {
  it("round-trips the supported discovery filters in canonical order", () => {
    const parsed = parseDiscoveryQuery(new URLSearchParams(
      "q=diving&state=Sabah&category=activity&category=food&type=activity%3Anature&type=food%3Aseafood&priceMax=150&free=1&bookable=1&hiddenGem=1&family=1&couple=1",
    ));
  ...
  it("trims strings, rejects invalid price limits, and omits defaults", () => {
    const parsed = parseDiscoveryQuery(new URLSearchParams(
      "q=%20%20&state=%20Sabah%20&category=%20activity%20&category=activity&type=%20activity%3Anature%20&type=activity%3Anature&priceMax=-1&free=0&bookable=true&hiddenGem=yes&family=0&couple=true",
    ));
    ...
    expect(serializeDiscoveryQuery(parsed).toString()).toBe("state=Sabah&category=activity&type=activity%3Anature");
  });
});
