-- Vendor registration as ONE atomic transaction.
--
-- Before this, POST /api/vendors ran three separate .insert() calls
-- (vendors -> vendor_onboarding_profiles -> nothing). They were not in a
-- transaction, so a failure part-way left an orphan vendor, and no initial
-- outlet was created at all — a new vendor could not list products until they
-- manually created one.
--
-- This RPC creates vendor + onboarding profile + first outlet together, or
-- none of them. It also takes a per-user advisory lock and re-checks for an
-- existing vendor inside that lock, so a double-clicked Register button cannot
-- create two vendors.
--
-- Ownership model: the vendor is the brand/HQ, the outlet is what actually
-- trades. Every vendor therefore starts with exactly one outlet.
...
BEGIN
  ...
  -- Serialise registration per user so a double submit cannot race.
  ...
  -- Re-check INSIDE the lock: the first request may have committed already.
  IF EXISTS (
  ...
  -- ── slug: caller value, else slugified name; de-duplicate with -2, -3, … ──
  ...
  -- ── first outlet: the brand always starts with one trading location ──
  ...
  -- display_id is assigned by the set_outlet_display_id BEFORE INSERT trigger.
