-- Two-layer product model, step 1 of 4: ADDITIVE ONLY.
--
--   Vendor Product (shared identity: name, description, images, variants)
--   └── Outlet Offer (what the outlet actually sells: price, status)
--
-- Nothing reads the new structures yet — this step only creates them and
-- backfills from today's data, so behaviour is unchanged after deploying it.
--
-- Why products.outlet_id becomes NULLABLE:
--   products.outlet_id has ON DELETE CASCADE to outlets. If a shared product
--   kept pointing at one "home" outlet, closing that outlet would DELETE the
--   shared product and with it every other outlet's offer. Shared products
--   therefore carry outlet_id = NULL and belong to no single outlet.
--
-- Transaction-layer columns (inventory / cart_items / checkout_reservations)
-- are added for ALL product types, not just food. Branching the money path on
-- product_type would be a hidden switch in the most dangerous place; instead
-- every row gains outlet_id and checkout is changed exactly once.

-- ── 1. products.outlet_id nullable ──────────────────────────────────────────
ALTER TABLE public.products ALTER COLUMN outlet_id DROP NOT NULL;

-- Trigger must tolerate NULL (shared product owned by no single outlet).
...
BEGIN
  IF NEW.outlet_id IS NULL THEN
    RETURN NEW;  -- shared vendor-level product; ownership lives in outlet_offers
  END IF;
...
-- An offer may only join a product and an outlet owned by the SAME vendor.
...
-- ── 3. RLS on outlet_offers ─────────────────────────────────────────────────
ALTER TABLE public.outlet_offers ENABLE ROW LEVEL SECURITY;

-- Customers: only active offers of active+approved products at live outlets.
...
-- Vendor owner: full control over their own vendor's offers.
...
-- Outlet manager: only offers of the outlets they are assigned to.
...
-- ── 4. transaction layer gains outlet_id (all product types) ────────────────
...
-- Backfill from the product's current outlet.
...
-- Inventory identity becomes (variant, outlet) so two outlets never share stock.
