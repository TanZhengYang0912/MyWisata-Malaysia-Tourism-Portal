-- Pahang place model — seed data.
-- See docs/plans/2026-08-13-0106-pahang-place-seed-and-imagery.md
--
-- BUSINESS IDENTITIES ARE REAL. COMMERCIAL DATA IS NOT.
--
-- Vendor names, outlet names, addresses and coordinates are real Pahang
...
-- is a real Pahang business sourced from OpenStreetMap. The guide-service
-- products belong to 4 real Cameron Highlands / Taman Negara travel
-- agencies (shop=travel_agency); their itineraries and prices are plausible
-- demo content, not those agencies' actual catalogues. Their outlet_id IS
-- NULL is deliberate — see plan §2.3.
--
-- Divergence from the plan as written (2026-08-14):
--   - The plan's original D5 (4 fictional guide vendors) was superseded
--     project-wide: every vendor here is real. See plan header + §2.3.
--   - Vendor 21 (Sungai Lembing craft shop) does not exist in this file —
--     Overpass found zero shop=* nodes of substance in Sungai Lembing beyond
--     a convenience store, confirming §1.3's thin-coverage finding. Per D4,
--     it and its product (Pewter Tin-Mining Souvenir) are cut, not invented.
--   - Vendors 19-20 (Pine Beach hotel, Cameron Valley Tea Shop) and the 4
--     operator outlets were resolved via Overpass on 2026-08-14; 4 of the
--     resolved operator POI coordinates also corrected drift of 185m-5.03km
--     against the plan's first-draft guesses (mossy-forest, canopy-walkway,
--     genting-skyway, tioman-marine-park) — see plan §3.1.
--   - Final counts: 20 vendors / 20 outlets / 31 products (plan originally
--     drafted 21/17/32, revised through two rounds to this).
--   - category_id resolved once via c_food/c_activity/c_accommodation/
...
BEGIN
  ...
  VALUES
    ...
    -- Priced but unmanaged — deliberate, see plan D9 / §2.4. Not a missing operator.
  ...
  -- Guide services — outlet_id NULL, place-bound only. Real vendors (4
  -- licensed travel agencies), fictional itinerary names — see plan §2.3.
  INSERT INTO products (id, vendor_id, outlet_id, category_id, name, slug, description, product_type, requires_booking, base_price)
  VALUES
  ...
  -- names (see header disclaimer). Excludes the 6 guide products (outlet_id
