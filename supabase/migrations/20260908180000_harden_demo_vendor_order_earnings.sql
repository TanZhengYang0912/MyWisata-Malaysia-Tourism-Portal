-- Harden lecturer-demo Vendor earnings against malformed Product/Outlet links.
...
BEGIN
  ...
  WHERE vendor.id = p_vendor_id
    ...
    AND EXISTS (
      ...
      WHERE assignment.user_id = vendor.owner_id
        AND role.name = 'vendor_owner'
    );
  ...
  JOIN public.products AS product
   ...
   AND product.vendor_id = p_vendor_id
  ...
  IF v_transaction_id IS NULL THEN
    ...
    WHERE transaction.user_id = v_owner_id
      AND transaction.wallet_id = v_wallet_id
      AND transaction.order_id = p_order_id
      AND transaction.idempotency_key = v_idempotency_key
      AND transaction.type = 'earnings'
      AND transaction.amount_sen = v_amount_sen
      AND transaction.bucket = 'earnings'
      AND transaction.direction = 'credit';

    IF v_transaction_id IS NULL THEN
      RAISE EXCEPTION 'existing_demo_earning_mismatch';
    END IF;
