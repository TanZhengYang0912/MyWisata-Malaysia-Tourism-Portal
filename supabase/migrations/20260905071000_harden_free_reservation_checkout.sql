-- Harden RM0 checkout against client-supplied price and cart-line forgery.
-- This keeps the same public signature and successful checkout behavior.
...
BEGIN
  ...
  FOR v_line IN SELECT * FROM jsonb_to_recordset(p_lines) AS x(
    ...
    IF v_is_free THEN
      v_authoritative_price := CASE
        WHEN v_product_booking AND v_slot.price_override IS NOT NULL THEN v_slot.price_override
        ELSE v_product.base_price + v_variant.price_offset
      END;
