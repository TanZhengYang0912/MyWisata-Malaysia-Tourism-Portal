...
--    SECURITY DEFINER + STABLE, same pattern as is_admin() in 003 —
--    queries run inside these bypass RLS on the tables they touch, so
--    referencing them from a policy does not re-trigger that table's
--    own RLS policy (which is what caused the original 42P17 loop).
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 3. Recreate the two policies disable-rls.sql dropped, now recursion-safe
-- ─────────────────────────────────────────────────────────────
