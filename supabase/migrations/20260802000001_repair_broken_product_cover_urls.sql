-- Repair the five seeded catalogue rows whose Unsplash cover URLs return 404.
-- The old-URL guard keeps this idempotent and avoids overwriting later vendor edits.
...
WHERE p.status = 'active'
  ...
  AND p.name IN (
    ...
    'Live Jazz & Whisky Bar',
    'Waterfront Live Music Bar'
  )
  AND p.cover_url IN (
