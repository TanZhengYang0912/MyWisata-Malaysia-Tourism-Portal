-- ============================================================
-- 014_reward_calculation.sql — Reward Calculation System
--
-- Adds:
--   · wallets.pending_earnings_sen  (hold bucket, not yet withdrawable)
--   · wallet_transactions type/bucket enum extension
--   · affiliate_attributions: hold_until, confirmed_at, reversed_at
--   · affiliate_links: UNIQUE(user_id) guard
--   · RPCs: credit_pending_earnings, confirm_pending_earnings,
--            reverse_pending_earnings, gen_affiliate_code
--   · Commission tier rules: Bronze/Silver/Gold + recommendation
--   · platform_settings: earnings.hold_days = 7
--
-- Flow:
--   order paid → credit_pending_earnings (→ pending_earnings_sen)
--   after hold_until → confirm_pending_earnings (→ earnings_sen)
--   on refund → reverse_pending_earnings (pending only; confirmed untouched)
-- ============================================================


-- ── 1. wallets.pending_earnings_sen ──────────────────────────────────────────
-- Money entering pending state (commission held for N days before clearing).
-- NOT withdrawable until confirmed → earnings_sen.
ALTER TABLE wallets
...
ALTER TABLE wallet_transactions ADD CONSTRAINT wallet_transactions_type_check
  CHECK (type IN (
    ...
    'withdrawal_complete',
    'withdrawal_cancel',
    'earnings_pending',   -- commission credited into pending bucket
    'earnings_confirm',   -- pending → available (debit pending / credit earnings)
    'earnings_reverse'    -- pending commission reversed (refund case)
  ));
...
-- ── 4. affiliate_attributions — hold window columns ──────────────────────────
ALTER TABLE affiliate_attributions
  ADD COLUMN IF NOT EXISTS hold_until   TIMESTAMPTZ,  -- NULL = already cleared (legacy rows)
  ADD COLUMN IF NOT EXISTS confirmed_at TIMESTAMPTZ,  -- set when status → 'confirmed'
  ADD COLUMN IF NOT EXISTS reversed_at  TIMESTAMPTZ;  -- set when status → 'reversed'


-- ── 5. affiliate_links — one link per user (prevent dup race) ────────────────
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    -- Remove duplicates (keep the oldest row per user) before adding constraint.
...
-- ── 6. credit_pending_earnings — called when a commission is first earned ─────
--    Puts money in pending_earnings_sen (not yet withdrawable).
--    The attribution's hold_until controls when confirm_pending_earnings clears it.
...
-- ── 7. confirm_pending_earnings — bulk clearance job ─────────────────────────
--    Moves all past-due pending attributions into available earnings_sen.
--    Called by Vercel Cron (service_role) or admin UI (authenticated admin).
--    Returns the count of attributions confirmed.
...
BEGIN
  ...
  LOOP
    ...
    -- Safety: never deduct more than what's in pending bucket
...
-- Admin can trigger from UI; cron uses service_role (which bypasses this grant).
GRANT EXECUTE ON FUNCTION confirm_pending_earnings() TO authenticated;


-- ── 8. reverse_pending_earnings — refund hook; pending only ──────────────────
--    Reverses a single pending attribution. If already confirmed, does nothing
--    (confirmed earnings are earned and kept — see ADR-016).
--    Returns TRUE if reversed, FALSE if not found or already confirmed/reversed.
...
-- ── 9. gen_affiliate_code — idempotent affiliate link creation ────────────────
--    Called from setVerificationTier when a user reaches 'kyc_verified'.
--    Returns the existing code if a link already exists (race-safe via UNIQUE).
...
BEGIN
  ...
  -- Generate a new unique code, retrying on collision
  LOOP
...
-- ── 10. Commission tier rules ─────────────────────────────────────────────────
-- Deactivate the flat-rate 'standard' rule from migration 011 so tier lookup
-- (min_conversions DESC) doesn't pick it over the new Bronze/Silver/Gold rules.
UPDATE commission_rules
   SET is_active = FALSE
 WHERE rule_type = 'affiliate' AND tier_name = 'standard';

-- Bronze → Silver → Gold tiers (rolling 30-day confirmed conversions)
...
-- Recommendation commission rule (first-sale bonus + ongoing rate)
INSERT INTO commission_rules (name, rule_type, one_time_bonus, ongoing_rate, tier_name, min_conversions)
