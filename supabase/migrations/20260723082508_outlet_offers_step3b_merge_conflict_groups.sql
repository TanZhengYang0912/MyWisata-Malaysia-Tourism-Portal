-- Resolves the 5 variant-conflict food groups left by step 3, using the
-- intersection rule chosen by the product owner: keep only the variant every
-- outlet has (Standard), drop the ones only some outlets had (Child).
--
-- Verified safe first: the Child variants had zero order_items, zero cart_items,
-- zero reservations and zero inventory — they were never sellable. Nothing in
-- order history references them, so removing them cannot alter past orders.
--
-- After the variant sets match, the same verified merge as step 3 runs: outlets
-- become offers on one shared product, and the shared product's outlet_id is
-- set to NULL so closing an outlet cannot cascade-delete it.
...
DELETE FROM public.product_variants v
 ...
 WHERE p.id = v.product_id
   ...
   AND v.name <> 'Standard'
   AND NOT EXISTS (SELECT 1 FROM public.order_items   o WHERE o.variant_id = v.id)
   AND NOT EXISTS (SELECT 1 FROM public.cart_items    c WHERE c.variant_id = v.id)
   AND NOT EXISTS (SELECT 1 FROM public.checkout_reservations r WHERE r.variant_id = v.id);
...
BEGIN
  FOR g IN
    WITH sig AS (
      SELECT p.id, p.vendor_id, p.name, p.category_id, p.requires_booking,
             array_to_string(p.type_slugs, ',') AS tslugs,
  ...
  LOOP
    ...
    LOOP
      ...
      UPDATE public.inventory i SET variant_id = cv.id
        ...
        JOIN public.product_variants cv ON cv.product_id = v_canonical AND cv.name = ov.name
