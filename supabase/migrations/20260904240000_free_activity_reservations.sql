-- Free Activity Reservations and Capacity Control
-- Allows RM0 capacity-constrained activities to be reserved without payment processing.
...
-- Keep 11-argument overload in sync
CREATE OR REPLACE FUNCTION public.prepare_checkout(
  p_cart_id UUID,
  p_selected_item_ids UUID[],
  p_idempotency_key TEXT,
...
-- Update sample activity (Merdeka Square Heritage Walk) to bookable free experience
UPDATE public.products
   SET requires_booking = true,
       base_price = 0
 WHERE slug = 'merdeka-square-heritage-walk';
