...
-- Build one source entitlement per customer/outlet. Prefer an old outlet-bound
-- claim when a product-level claim came from the prior duplicate dataset; if
-- no hint exists, choose the first currently eligible outlet deterministically.
...
-- Preserve the previous product-level launch rows, but remove them from both
-- customer tabs so one outlet cannot render multiple duplicate cards.
