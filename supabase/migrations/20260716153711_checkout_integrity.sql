-- Checkout integrity follow-up. This migration is additive and preserves existing rows.
...
CREATE INDEX IF NOT EXISTS idx_checkout_reservations_cart_item
  ON public.checkout_reservations(cart_item_id);

UPDATE public.vouchers
   SET reserved_uses = 0
 WHERE reserved_uses IS NULL;

ALTER TABLE public.vouchers
  ALTER COLUMN reserved_uses SET DEFAULT 0,
  ALTER COLUMN reserved_uses SET NOT NULL;

-- Existing demo data may contain historically over-capacity slots. Keep every booking,
-- but make the availability state honest so new checkout attempts are blocked.
UPDATE public.booking_slots
   SET status = 'full'
 WHERE booked >= capacity AND status <> 'full';
...
BEGIN
  ...
  END IF;
  UPDATE public.booking_slots
     SET status = CASE WHEN booked >= capacity THEN 'full' ELSE 'available' END
   WHERE status IN ('available', 'full')
     AND ((booked >= capacity AND status <> 'full') OR (booked < capacity AND status = 'full'));
  GET DIAGNOSTICS v_updated = ROW_COUNT;
  RETURN v_updated;
END;
$$;

REVOKE ALL ON FUNCTION public.reconcile_booking_capacity() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.reconcile_booking_capacity() TO authenticated;

-- Remove the no-op cart update from the original function and retain exact cart item
-- identity for future successful finalization.
...
-- Release abandoned holds without deleting order/payment history.
...
DECLARE
  ...
  v_count INTEGER := 0;
BEGIN
  IF COALESCE(auth.jwt() ->> 'role', '') <> 'service_role' AND (auth.uid() IS NULL OR NOT is_admin(auth.uid())) THEN
    RAISE EXCEPTION 'admin_or_service_role_required';
  END IF;
  FOR v_session IN SELECT id FROM public.checkout_sessions WHERE status IN ('pending_payment','requires_action') AND expires_at < NOW() FOR UPDATE LOOP
    PERFORM public.finalize_checkout(v_session.id, 'expired');
    v_count := v_count + 1;
  END LOOP;
  RETURN v_count;
END;
...
REVOKE ALL ON FUNCTION public.expire_checkout_sessions() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.expire_checkout_sessions() TO authenticated, service_role;
;
