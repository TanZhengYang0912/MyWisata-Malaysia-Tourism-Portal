...
const sql = readFileSync(
  new URL("../20260829203000_progressive_verification_db_guards.sql", import.meta.url),
  "utf8",
);
...
describe("progressive verification database guards", () => {
  ...
  it("enforces Profile Complete or approved KYC on recommendation and affiliate inserts", () => {
    expect(sql).toContain("CREATE OR REPLACE FUNCTION public.enforce_recommendation_profile_complete()");
    expect(sql).toContain("CREATE TRIGGER vendor_recommendations_require_profile_complete");
  ...
  it("clears affiliate earnings only while the owner KYC row is locked and approved", () => {
    ...
    expect(sql).toContain("REVOKE ALL ON FUNCTION public.confirm_pending_earnings() FROM anon");
  });

  it("does not require Phone, Profile, or legacy tier for KYC-approved withdrawals", () => {
    const withdrawal = sql.slice(sql.indexOf("CREATE OR REPLACE FUNCTION public.enforce_progressive_withdrawal_tier()"));
    expect(withdrawal).toContain("u.kyc_status = 'approved'");
  ...
  it("removes direct access to obsolete withdrawal and wallet mutation paths", () => {
    expect(sql).toContain("REVOKE ALL ON FUNCTION public.submit_withdrawal(UUID, NUMERIC, UUID) FROM authenticated");
    expect(sql).toContain("REVOKE ALL ON FUNCTION public.reserve_for_withdrawal(UUID, BIGINT, UUID) FROM authenticated");
    expect(sql).toContain("REVOKE ALL ON FUNCTION public.cancel_withdrawal(UUID, UUID, TEXT) FROM authenticated");
    expect(sql).toContain("REVOKE ALL ON FUNCTION public.complete_withdrawal(UUID, UUID, TEXT) FROM authenticated");
    expect(sql).toContain("REVOKE ALL ON FUNCTION public.admin_set_processing(UUID, TEXT, TEXT) FROM authenticated");
  });
});
