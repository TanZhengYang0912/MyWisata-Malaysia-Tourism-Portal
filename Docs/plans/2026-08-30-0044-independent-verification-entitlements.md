...
**Status (2026-08-30):** Entitlement and Profile-fact repairs are implemented and locally verified. The approved UX simplification is also implemented: the existing Profile editor is the single Account destination, Phone/KYC appear as independent status cards above its four-step progress, and the duplicate intent-card hub is retired through a compatibility redirect. The effective Profile fact still requires its historical completion timestamp plus current Identity, Photo, About you, and Preferences evidence across application and database authorization boundaries. Historical completion timestamps are preserved; Phone and KYC remain independent and untouched. Disposable-local pgTAP execution remains environment-blocked because Docker/Podman is unavailable; no linked, shared, or remote database was reset or migrated.
...
- The Profile verification-card UX follow-up passed its affected TDD suite and browser checks at desktop and `390px`: Phone/KYC routes worked, the legacy verification route redirected without a loop, the original business banner and four-step Profile skeleton remained, and the final console error log was empty. The fresh full suite passed 506 files (7 skipped) and 2353 tests (20 skipped); TypeScript, diff-check, and i18n verification exited 0. Full ESLint exited 0 with the same 55 repository warnings and no new warning in `components/profile/verification-path-cards.tsx`.
- The final bounded `luna_worker` review approved the UX diff with no confirmed security, privacy, authorization, or broken-flow blocker. It confirmed that the cards expose only normalized low-sensitivity status, use the authenticated private Profile summary, do not modify entitlement/database/backend rules, and do not introduce an open redirect or redirect loop.
...
- Non-blocking UX follow-ups: the compatibility redirect currently drops legacy query parameters; if continuation support is ever required, forward only a value passed through the existing safe-next sanitizer. Completed Profile state may show the same Phone/KYC status in both the new cards and the preserved existing summary badges.
...
3. Each card shows its current state from the authenticated private Profile summary and links directly to `/customer/phone` or `/customer/kyc`. Pending and rejected KYC remain visibly distinct through the existing localized state strings.
...
**Risks:** The two cards could crowd the Profile page on mobile or show stale state after verification. Responsive contract/browser checks must cover the stacked layout, and the cards must read only the existing authenticated `/api/profile/me` summary refreshed by the Phone/KYC flows. The legacy redirect must not create a loop.
...
- [x] **Step 1 — RED: update the navigation and route contracts.** In `lib/customer/__tests__/header-navigation.test.ts`, require the only Account Profile destination to be `/customer/profile` and require zero `/customer/verification` menu items. In `app/customer/verification/__tests__/page.contract.test.ts`, require `redirect("/customer/profile")` and reject `INTENTS`, `capabilities`, and visible account-verification sections. Run both tests and confirm they fail against the current hub.
- [x] **Step 2 — GREEN: make Profile the canonical destination.** Change `lib/customer/header-navigation.ts` to point `Profile & Verification` to `/customer/profile`. Replace `app/customer/verification/page.tsx` with a server redirect that imports `redirect` from `next/navigation` and calls `redirect("/customer/profile")`. Rerun Step 1 tests and confirm they pass.
- [x] **Step 3 — RED: specify the compact independent-path component.** Extend `app/customer/profile/__tests__/profile-completion.test.ts` to require one `VerificationPathCards` render before the wizard progress in both completion branches. Add `components/profile/__tests__/verification-path-cards.test.ts` covering Phone/KYC routes, responsive layout, localized status states, and the absence of a visible section heading. Run the two tests and confirm the component/import is missing.
- [x] **Step 4 — GREEN: add and integrate the two-card row.** Create `components/profile/verification-path-cards.tsx` exporting `VerificationPathCards({ phoneVerified, kycStatus })`. It uses the existing `ui.accountVerification.statuses.*` locale keys, `Phone`, `ShieldCheck`, `CheckCircle2`, and `Circle`, with `grid gap-3 sm:grid-cols-2` and no visible heading. Modify `app/customer/profile/page.tsx` to use the authenticated private Profile summary and render the component immediately after `BusinessShareBanner` in both incomplete and complete branches. Rerun Step 3 tests and confirm they pass.
- [x] **Step 5 — affected verification.** Run `npx vitest run lib/customer/__tests__/header-navigation.test.ts app/customer/verification/__tests__/page.contract.test.ts app/customer/profile/__tests__/profile-completion.test.ts components/profile/__tests__/verification-path-cards.test.ts app/customer/__tests__/sitewide-i18n.contract.test.ts`, scoped ESLint, `npx tsc --noEmit`, and `git diff --check`.
- [x] **Step 6 — browser/design QA.** In the in-app browser, verify `/customer/profile` at desktop and narrow widths, click both cards, confirm the business banner and all four Profile steps are preserved, confirm `/customer/verification` redirects without a loop, and compare the Profile screenshot against the supplied reference. Record the result in `design-qa.md` with `final result: passed` before handoff.
- [x] **Step 7 — final verification and commit.** Run the full Vitest suite once, request one focused `luna_worker` read-only route/privacy review, update this plan status/evidence, and commit only the owned UI/test/plan files. No database or entitlement files are staged. Completed in the commit that records this checklist result.
