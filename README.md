...
- **TNG eWallet payouts:** configure the real TNG Direct Credit merchant credentials and provider adapter only after the official provider contract is available. The application derives the destination from the signed-in customer's OTP-verified Malaysian phone and never accepts a browser-supplied TNG identifier or PIN. The feature remains visibly unavailable until the provider is genuinely configured.
